...
impl RelationshipAnalyzer {
    ...
    pub fn determine_relationship_type(
        ...
        match (
            ...
            (i, t, _f) if i > 0.5 && t > 0.6 && (i - t).abs() > 0.3 => {
                Ok(RelationshipType::Mentorship)
            }
            (i, t, f) if f > 0.7 && i.abs() < 0.3 && t > 0.5 => Ok(RelationshipType::Professional),
            (i, t, _f) if i > 0.8 && t > 0.9 && self.has_family_markers(relationship) => {
                Ok(RelationshipType::Family)
            }
    ...
    fn determine_mood(&self, relationship: &Relationship) -> Result<Mood> {
        ...
        match (
            ...
            (i, _t, RelationshipType::Rivalry | RelationshipType::Competition) if i.abs() > 0.3 => {
                Ok(Mood::Competitive)
            }
    ...
    fn calculate_recent_trend(&self, history: &[RelationshipEvent]) -> Result<TrendDirection> {
        ...
        let total_intensity_change: f32 = recent_events
            .iter()
            .map(|e| e.impact.intensity_change)
            .sum();

        let total_trust_change: f32 = recent_events.iter().map(|e| e.impact.trust_change).sum();

        // Check for volatility (large swings)
        let intensity_variance: f32 = recent_events
            .iter()
            .map(|e| e.impact.intensity_change)
    ...
    fn calculate_stability(&self, history: &[RelationshipEvent]) -> Result<f32> {
        ...
        let intensity_changes: Vec<f32> =
            history.iter().map(|e| e.impact.intensity_change).collect();

        let mean_change = intensity_changes.iter().sum::<f32>() / intensity_changes.len() as f32;
        let variance = intensity_changes
            .iter()
            .map(|&change| (change - mean_change).powi(2))
            .sum::<f32>()
            / intensity_changes.len() as f32;
    ...
    pub fn determine_interaction_style(
        &self,
        relationship: &Relationship,
        ...
        match (
            ...
            (i, t, RelationshipType::Professional) if i > 0.0 && t > 0.5 => {
                Ok(InteractionStyle::Professional)
            }
            (i, t, _) if i > 0.4 && t > 0.6 => Ok(InteractionStyle::Supportive),
            (i, t, _) if i < 0.0 && t < 0.5 => Ok(InteractionStyle::Cautious),
            (i, _t, RelationshipType::Rivalry | RelationshipType::Competition) if i.abs() > 0.3 => {
                Ok(InteractionStyle::Aggressive)
            }
            ...
            (i, t, RelationshipType::Romance) if i > 0.3 && t > 0.5 => {
                Ok(InteractionStyle::Playful)
            }
