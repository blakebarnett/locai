...
pub type RelationshipEnrichmentFn = Box<dyn Fn(&str, &str, &str) -> HashMap<String, serde_json::Value> + Send + Sync>;
...
impl RelationshipManager {
    ...
    pub async fn initialize_relationship(&self, entity_a: &str, entity_b: &str) -> Result<String> {
        ...
        if let Ok(existing) = self.get_relationship_from_memory(entity_a, entity_b).await {
            debug!("Relationship between {} and {} already exists: {}", entity_a, entity_b, existing.id);
        ...
        let memory_id = self.memory_manager.add_memory_with_options(
            &format!("Relationship between {} and {}: {} (intensity: {:.2}, trust: {:.2})", 
                     entity_a, entity_b, relationship.relationship_type, 
                     relationship.intensity, relationship.trust_level),
            |builder| {
                let mut properties = HashMap::new();
                properties.insert("relationship_data", serde_json::to_value(&relationship).unwrap_or_default());
                properties.insert("relationship_id", serde_json::Value::String(relationship.id.clone()));
                ...
                builder
                    ...
                    .tags(vec![
                        ...
                        entity_a,
                        entity_b,
                        &format!("type_{}", relationship.relationship_type.to_string().to_lowercase()),
                    ])
                    .properties(properties)
            }
        ).await?;
        
        info!("🤝 Initialized relationship between {} and {} (ID: {})", entity_a, entity_b, relationship.id);
        Ok(memory_id)
    }
    ...
    pub async fn update_relationship(
        ...
        let mut relationship = match self.get_relationship_from_memory(entity_a, entity_b).await {
            ...
            Err(_) => {
                ...
                self.get_relationship_from_memory(entity_a, entity_b).await?
            }
        };
        ...
        relationship.intensity = (relationship.intensity + event.impact.intensity_change).clamp(-1.0, 1.0);
        relationship.trust_level = (relationship.trust_level + event.impact.trust_change).clamp(0.0, 1.0);
        relationship.familiarity = (relationship.familiarity + event.impact.familiarity_change).clamp(0.0, 1.0);
        ...
        if let Some(ref new_type) = event.impact.relationship_type_shift {
            ...
            if let Ok(suggested_type) = self.analyzer.determine_relationship_type(&relationship) {
                if suggested_type != relationship.relationship_type {
                    debug!("Relationship type evolved from {:?} to {:?} for {} and {}", 
        ...
        info!("🤝 Updated relationship between {} and {} (intensity: {:.2} → {:.2}, trust: {:.2} → {:.2})", 
              entity_a, entity_b, old_intensity, relationship.intensity, old_trust, relationship.trust_level);
        
        Ok(())
    }
    
    /// Get relationship context including emotional state
    pub async fn get_relationship_context(&self, entity_a: &str, entity_b: &str) -> Result<RelationshipContext> {
        let relationship = self.get_relationship_from_memory(entity_a, entity_b).await?;
        let recent_events = relationship.get_recent_events(24).into_iter().cloned().collect();
    ...
    pub async fn get_entity_relationships(&self, entity: &str) -> Result<Vec<Relationship>> {
        let memories = self.memory_manager.search_memories(&format!("relationship {}", entity), None).await?;
        
        let mut relationships = Vec::new();
        for memory in memories {
            if let Some(relationship_data) = memory.properties.get("relationship_data") {
                if let Ok(relationship) = serde_json::from_value::<Relationship>(relationship_data.clone()) {
    ...
    pub async fn process_entity_action(
        ...
        for other_entity in other_entities {
            if entity != other_entity {
                ...
                let event = self.create_event_from_action(action, magnitude, context, enrichment_data);
                self.update_relationship(entity, other_entity, event).await?;
            }
        }
    ...
    pub async fn get_relationship_summary(&self, entity_a: &str, entity_b: &str) -> Result<String> {
        ...
        let summary = format!(
            ...
            entity_a, entity_b, rel.relationship_type,
            rel.intensity, rel.trust_level,
    ...
    async fn get_relationship_from_memory(&self, entity_a: &str, entity_b: &str) -> Result<Relationship> {
        let search_query = format!("relationship {} {}", entity_a, entity_b);
        let memories = self.memory_manager.search_memories(&search_query, None).await?;
        ...
        Err(anyhow!("Relationship not found between {} and {}", entity_a, entity_b))
    }
    
    async fn store_relationship_update(&self, relationship: &Relationship) -> Result<()> {
        // Find and update existing memory
        let search_query = format!("relationship {} {}", relationship.entity_a, relationship.entity_b);
        let memories = self.memory_manager.search_memories(&search_query, None).await?;
                ...
                if let Some(id_str) = stored_id.as_str() {
                    if id_str == relationship.id {
                        ...
                        if let serde_json::Value::Object(ref mut map) = updated_memory.properties {
                            map.insert("relationship_data".to_string(), serde_json::to_value(relationship).unwrap_or_default());
                        }
                        
                        // Update memory content
                        updated_memory.content = format!(
                            "Relationship between {} and {}: {} (intensity: {:.2}, trust: {:.2})",
                            relationship.entity_a, relationship.entity_b, relationship.relationship_type,
                            relationship.intensity, relationship.trust_level
                        );
        ...
        warn!("Could not find memory to update for relationship {}", relationship.id);
        Ok(())
    }
    ...
    fn calculate_action_magnitude(&self, action: &str) -> f32 {
        ...
        if action_lower.contains("attack") || action_lower.contains("betray") || action_lower.contains("destroy") {
            0.9
        } else if action_lower.contains("help") || action_lower.contains("save") || action_lower.contains("support") {
            0.8
        } else if action_lower.contains("argue") || action_lower.contains("disagree") || action_lower.contains("conflict") {
            0.6
        } else if action_lower.contains("cooperate") || action_lower.contains("collaborate") || action_lower.contains("assist") {
            0.7
        } else if action_lower.contains("talk") || action_lower.contains("discuss") || action_lower.contains("chat") {
            0.3
        } else {
    ...
    fn create_event_from_action(
        ...
        let impact = if let Some(sentiment_value) = enrichment_data.get("sentiment") {
            ...
            } else if let Ok(sentiment_score) = serde_json::from_value::<f32>(sentiment_value.clone()) {
        ...
        let mut event = RelationshipEvent::new(
            event_type,
            action.to_string(),
            impact,
            context.to_string(),
    ...
    fn determine_event_type_from_keywords(&self, action: &str) -> EventType {
        ...
        if action_lower.contains("help") || action_lower.contains("support") {
            ...
            EventType::Conflict
        } else if action_lower.contains("love") || action_lower.contains("praise") || action_lower.contains("thank") {
            EventType::PositiveInteraction
        } else if action_lower.contains("insult") || action_lower.contains("mock") || action_lower.contains("hurt") {
    ...
    fn determine_impact_from_keywords(&self, action: &str, magnitude: f32) -> RelationshipImpact {
        let action_lower = action.to_lowercase();
        if action_lower.contains("help") || action_lower.contains("support") || 
           action_lower.contains("love") || action_lower.contains("praise") {
            RelationshipImpact::positive_interaction(magnitude * 0.7)
        } else if action_lower.contains("attack") || action_lower.contains("betray") || 
                  action_lower.contains("insult") || action_lower.contains("hurt") {
