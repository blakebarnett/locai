...
impl ConfigBuilder {
    ...
    pub fn with_default_storage(mut self) -> Self {
        ...
        self.config.storage.graph.surrealdb = SurrealDBConfig {
            ...
            connection: self.config.storage.data_dir.join("graph").to_string_lossy().to_string(),
    ...
    pub fn without_entity_extraction(mut self) -> Self {
        ...
        self.config.entity_extraction.automatic_relationships.enabled = false;
        self
    }
    
    /// Configure entity extraction settings.
    pub fn with_entity_extraction_config(mut self, config: crate::entity_extraction::EntityExtractionConfig) -> Self {
    ...
    pub fn with_remote_surrealdb_if_configured(mut self) -> Self {
        if let Ok(connection_url) = std::env::var("SURREALDB_URL") {
            tracing::info!("Configuring SurrealDB remote connection to: {}", connection_url);
            
            // Determine engine type from URL
            let engine = if connection_url.starts_with("ws://") || connection_url.starts_with("wss://") {
                SurrealDBEngine::WebSocket
            } else if connection_url.starts_with("http://") || connection_url.starts_with("https://") {
                SurrealDBEngine::Http
            } else {
                SurrealDBEngine::Http // Default to HTTP for remote connections
            };
            
            let namespace = std::env::var("SURREALDB_NAMESPACE").unwrap_or_else(|_| "locai".to_string());
