...
//! let embedding = get_embedding_from_openai("text").await?;
//! manager.validate_embedding(&embedding)?;
//! 
//! // Normalize if needed
//! let mut embedding = get_embedding_from_provider("text").await?;
...
impl EmbeddingManager {
        ...
        if let Some(expected_dim) = self.expected_dimensions {
            if embedding.len() != expected_dim {
                return Err(MLError::embedding(
                    format!("Expected embedding dimension {}, got {}", expected_dim, embedding.len())
                ));
            }
        }
    ...
    pub fn normalize_embedding(&self, embedding: &mut [f32]) -> Result<()> {
        ...
        if norm == 0.0 {
            return Err(MLError::embedding("Cannot normalize zero vector".to_string()));
        }
