    ...
    C: Connection + Clone + Send + Sync + 'static,
{
    ...
    pub async fn start_live_queries(&mut self) -> Result<(), StorageError> {
        info!("Starting SharedStorage live queries for node {}", self.node_id);
        ...
        match self.client.query(&query).await {
            Ok(mut response) => {
                ...
                if let Ok(Some(live_uuid)) = response.take::<Option<Uuid>>(0) {
                    info!("Live query created for table {} with UUID: {}", table, live_uuid);
                    ...
                    self.process_live_query_events(live_uuid, table.to_string()).await;
    ...
    async fn process_live_query_events(&self, live_uuid: Uuid, table: String) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match Self::process_live_query_stream(&client, live_uuid, &table, &event_tx, &node_id).await {
                    Ok(_) => {
                    ...
                    Err(e) => {
                        ...
                        if retry_count > MAX_RETRIES {
                            error!("Live query for table {} failed after {} retries: {}", table, MAX_RETRIES, e);
                            break;
                        }
                        
                        let delay = INITIAL_RETRY_DELAY * 2_u32.pow(retry_count as u32 - 1);
                        warn!("Live query for table {} failed (attempt {}), retrying in {:?}: {}", 
                    ...
                    if let Ok(records) = response.take::<Vec<Value>>(0) {
                        ...
                        for record in records {
                            ...
                            let db_event = DbEvent {
                                id: format!("{}_{}", live_uuid, chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0)),
                                action,
    ...
    fn determine_action_type(record: &Value) -> String {
        if let (Some(created), Some(updated)) = (
            ...
            if let (Ok(created_time), Ok(updated_time)) = (
                ...
                let diff = updated_time.signed_duration_since(created_time).num_milliseconds();
                if diff.abs() < 1000 { // Within 1 second
                    "CREATE".to_string()
                } else {
                    "UPDATE".to_string()
                }
    ...
    pub fn simulate_event(&self, table: &str, action: &str, data: Value) -> Result<(), StorageError> {
        let db_event = DbEvent {
        ...
        self.event_tx.send(db_event)
            .map_err(|e| StorageError::Connection(format!("Failed to send simulated event: {}", e)))?;
