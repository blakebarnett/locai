...
use crate::storage::traits::{BaseStore, GraphStore, GraphTraversal, MemoryStore, EntityStore, RelationshipStore};
use crate::storage::models::{MemoryGraph, MemoryPath, Entity, Relationship};
    ...
    C: Connection + Clone + Send + Sync + std::fmt::Debug + 'static,
{
    ...
    async fn setup_live_queries(&self) -> Result<Option<Box<dyn std::any::Any + Send>>, StorageError> {
    ...
    C: Connection + Clone + Send + Sync + std::fmt::Debug + 'static,
{
    ...
    async fn get_memory_subgraph(&self, memory_id: &str, depth: u8) -> Result<MemoryGraph, StorageError> {
        // First verify the central memory exists
        let central_memory = self.get_memory(memory_id).await?
        ...
        self.collect_memory_subgraph_recursive(&mut graph, memory_id, depth, 0, &mut HashSet::new()).await?;
    ...
    async fn find_paths(&self, from_id: &str, to_id: &str, max_depth: u8) -> Result<Vec<MemoryPath>, StorageError> {
        // Verify both memories exist
        let _from_memory = self.get_memory(from_id).await?
            .ok_or_else(|| StorageError::NotFound(format!("Source memory {} not found", from_id)))?;
        let _to_memory = self.get_memory(to_id).await?
    ...
    async fn find_connected_memories(&self, memory_id: &str, relationship_type: &str, max_depth: u8) -> Result<Vec<Memory>, StorageError> {
        // Verify the source memory exists
        let _source_memory = self.get_memory(memory_id).await?
            .ok_or_else(|| StorageError::NotFound(format!("Memory {} not found", memory_id)))?;
        ...
        while let Some((current_memory_id, current_depth)) = queue.pop_front() {
            ...
            for entity in entities {
                ...
                let related_entities = self.find_related_entities(&entity.id, Some(relationship_type.to_string()), Some("both".to_string())).await?;
                
                for related_entity in related_entities {
                    // Get memories that contain this related entity
                    let memories = self.get_memories_containing_entity(&related_entity.id).await?;
    ...
    async fn get_entities_from_memory(&self, memory_id: &str) -> Result<Vec<Entity>, StorageError> {
        ...
        let mut response = self.client
            ...
            .await
            .map_err(|e| StorageError::Query(format!("Failed to get entities from memory: {}", e)))?;
    ...
    async fn get_memories_containing_entity(&self, entity_id: &str) -> Result<Vec<Memory>, StorageError> {
        let query = r#"
        ...
        let mut response = self.client
            ...
            .await
            .map_err(|e| StorageError::Query(format!("Failed to get memories containing entity: {}", e)))?;
    ...
    async fn get_entity_relationships(&self, entity_id: &str) -> Result<Vec<Relationship>, StorageError> {
        let query = r#"
            SELECT * FROM relates 
        ...
        let mut response = self.client
            ...
            .bind(("entity_id", entity_id_owned))
            .await
            .map_err(|e| StorageError::Query(format!("Failed to get entity relationships: {}", e)))?;
    ...
    C: Connection + Clone + Send + Sync + std::fmt::Debug + 'static,
{
    ...
    async fn collect_memory_subgraph_recursive(
        ...
        for entity in entities {
            ...
            for relationship in relationships {
                ...
                let connected_memories = self.get_memories_containing_entity(other_entity_id).await?;
    ...
    async fn find_paths_bfs(&self, from_id: &str, to_id: &str, max_depth: u8) -> Result<Vec<MemoryPath>, StorageError> {
            ...
            for entity in entities {
                ...
                for relationship in relationships {
                    ...
                    for connected_memory in connected_memories {
                        ...
                        if current_path.memories.iter().any(|m| m.id == connected_memory.id) {
                            continue;
                        }
