    ...
    C: Connection + Clone + Send + Sync + std::fmt::Debug + 'static,
{
            ...
            self.client
                ...
                .map_err(|e| {
                    StorageError::Query(format!(
                        "Failed to create vector with ID {}: {}",
                        vector.id, e
                    ))
                })?
        } else {
    ...
    async fn get_vector(&self, id: &str) -> Result<Option<Vector>, StorageError> {
        ...
        let vector: Option<SurrealVector> = self
            .client
            .select(("vector", id))
            .await
    ...
    async fn delete_vector(&self, id: &str) -> Result<bool, StorageError> {
        let deleted: Option<SurrealVector> = self
            .client
            .delete(("vector", id))
            .await
    ...
    async fn update_vector_metadata(
        ...
        let updated: Option<SurrealVector> = self
            .client
            .update(("vector", id))
            .merge(update_data)
            .await
    ...
    async fn search_vectors(
        ...
        if let Some(filter) = &params.filter {
            ...
            if filter.metadata.is_some() {
                tracing::warn!(
                    "Metadata filtering in vector search not yet implemented with separate metadata table"
                );
            }
        }
    ...
    async fn list_vectors(
        ...
        if filter.is_none() && limit.is_none() && offset.is_none() {
            let vectors: Vec<SurrealVector> = self
                .client
                .select("vector")
                .await
    ...
    async fn upsert_vector(&self, vector: Vector) -> Result<(), StorageError> {
        ...
        let _result: Option<SurrealVector> = self
            .client
