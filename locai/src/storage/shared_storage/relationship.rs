...
use crate::storage::traits::{RelationshipStore, EntityStore, MemoryStore};
...
impl From<Relationship> for SurrealRelationship {
    fn from(relationship: Relationship) -> Self {
        Self {
            id: RecordId::from(("relationship", relationship.id.as_str())).into(),
    ...
    C: Connection + Clone + Send + Sync + std::fmt::Debug + 'static,
{
    ...
    async fn create_relationship(&self, relationship: Relationship) -> Result<Relationship, StorageError> {
        ...
        match relationship.relationship_type.as_str() {
            "contains" | "mentions" | "references_entity" | "has_entity" => {
                ...
                if self.get_memory(&relationship.source_id).await?.is_none() {
                    return Err(StorageError::NotFound(format!("Source memory with ID {} not found", relationship.source_id)));
                }
                
                // Verify target is an entity
                if self.get_entity(&relationship.target_id).await?.is_none() {
                    return Err(StorageError::NotFound(format!("Target entity with ID {} not found", relationship.target_id)));
                }
            },
            "references" => {
                ...
                if self.get_relationship(&relationship.target_id).await?.is_none() {
                    return Err(StorageError::NotFound(format!("Target relationship with ID {} not found", relationship.target_id)));
                }
            },
            "entity_coreference" | "temporal_sequence" | "topic_similarity" => {
                ...
                if self.get_memory(&relationship.target_id).await?.is_none() {
                    return Err(StorageError::NotFound(format!("Target memory with ID {} not found", relationship.target_id)));
                }
            },
            _ => {
                ...
                if self.get_entity(&relationship.source_id).await?.is_none() {
                    return Err(StorageError::NotFound(format!("Source entity with ID {} not found", relationship.source_id)));
                }
        ...
        let created: Option<SurrealRelationship> = self.client
            .create("relationship")
        ...
        match relationship.relationship_type.as_str() {
            "contains" | "mentions" | "references_entity" | "has_entity" => {
                ...
                let _edge_result = self.client
                    .query(edge_query)
                    .bind(("source_memory", RecordId::from(("memory", source_id.as_str()))))
                    .bind(("target_entity", RecordId::from(("entity", target_id.as_str()))))
                    .bind(("relationship_type", rel_type))
                    .bind(("properties", props))
                    .await
                    .map_err(|e| StorageError::Query(format!("Failed to create contains edge: {}", e)))?;
            },
            "references" => {
                ...
                let _edge_result = self.client
                    .query(edge_query)
                    .bind(("source_memory", RecordId::from(("memory", source_id.as_str()))))
                    .bind(("target_relationship", RecordId::from(("relationship", target_id.as_str()))))
                    .bind(("relationship_type", rel_type))
                    .bind(("properties", props))
                    .await
                    .map_err(|e| StorageError::Query(format!("Failed to create references edge: {}", e)))?;
            },
            _ => {
                ...
                let _edge_result = self.client
                    .query(edge_query)
                    .bind(("source_entity", RecordId::from(("entity", source_id.as_str()))))
                    .bind(("target_entity", RecordId::from(("entity", target_id.as_str()))))
                    .bind(("relationship_type", rel_type))
                    .bind(("properties", props))
                    .await
                    .map_err(|e| StorageError::Query(format!("Failed to create relates edge: {}", e)))?;
            }
        }
    ...
    async fn get_relationship(&self, id: &str) -> Result<Option<Relationship>, StorageError> {
        let relationship: Option<SurrealRelationship> = self.client
    ...
    async fn update_relationship(&self, relationship: Relationship) -> Result<Relationship, StorageError> {
        ...
        let mut response = self.client
            .query(merge_query)
            .bind(("record_id", RecordId::from(("relationship", relationship.id.as_str()))))
        ...
        let updated: Option<SurrealRelationship> = response
            .take(0)
            .map_err(|e| StorageError::Query(format!("Failed to extract updated relationship: {}", e)))?;
        
        let updated_relationship = updated
            .map(Relationship::from)
            .ok_or_else(|| StorageError::NotFound(format!("Relationship with id {} not found", relationship.id)))?;
    ...
    async fn delete_relationship(&self, id: &str) -> Result<bool, StorageError> {
        ...
        let deleted: Option<SurrealRelationship> = self.client
        ...
        if deleted.is_some() {
            ...
            let _edge_result: Option<Value> = self.client
                ...
                .take(0)
                .map_err(|e| StorageError::Query(format!("Failed to extract edge delete result: {}", e)))?;
        }
    ...
    async fn list_relationships(
        ...
        if filter.is_none() && limit.is_none() && offset.is_none() {
            let relationships: Vec<SurrealRelationship> = self.client
                .select("relationship")
                .await
    ...
    async fn count_relationships(&self, filter: Option<RelationshipFilter>) -> Result<usize, StorageError> {
    ...
    async fn get_relationship_by_entities(&self, source_id: &str, target_id: &str) -> Result<Option<Relationship>, StorageError> {
        let query = r#"
        ...
        let mut response = self.client
            ...
            .bind(("target_id", target_id))
            .await
            .map_err(|e| StorageError::Query(format!("Failed to get relationship by entities: {}", e)))?;
    ...
    async fn get_relationship_properties(&self, id: &str) -> Result<Value, StorageError> {
        ...
        match relationship {
            ...
            None => Err(StorageError::NotFound(format!("Relationship with ID {} not found", id))),
        }
    }
    ...
    async fn find_related_entities(&self, entity_id: &str, relationship_type: Option<String>, direction: Option<String>) 
        ...
        if direction == "outgoing" || direction == "both" {
            ...
            let mut response = query_builder
                .await
                .map_err(|e| StorageError::Query(format!("Failed to find outgoing related entities: {}", e)))?;
        ...
        if direction == "incoming" || direction == "both" {
            ...
            let mut response = query_builder
                .await
                .map_err(|e| StorageError::Query(format!("Failed to find incoming related entities: {}", e)))?;
        ...
        // Remove duplicates
        all_entity_ids.sort_by(|a, b| a.key().to_string().cmp(&b.key().to_string()));
