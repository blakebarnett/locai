...
pub use types::{Message, MessageId, MessageFilter, MessageBuilder};
...
impl LocaiMessaging {
    ...
    pub async fn send(&self, topic: &str, content: serde_json::Value) -> Result<MessageId> {
        match &self.mode {
            MessagingMode::Embedded { memory_manager } => {
                embedded::send_message(memory_manager, &self.namespace, &self.app_id, topic, content).await
            }
            MessagingMode::Remote { websocket_client, .. } => {
    ...
    pub async fn subscribe_filtered(&self, filter: MessageFilter) -> Result<MessageStream> {
        match &self.mode {
            ...
            MessagingMode::Remote { websocket_client, .. } => {
                self.subscribe_filtered_remote(websocket_client, filter).await
            }
        }
    }
    ...
    pub async fn send_with_options(&self, message: Message) -> Result<MessageId> {
        match &self.mode {
            ...
            MessagingMode::Remote { websocket_client, .. } => {
                self.send_complete_message_remote(websocket_client, message).await
            }
        }
    }
    ...
    /// # Returns
    /// Message ID of the sent message
    pub async fn send_to_app(&self, target_app: &str, topic: &str, content: serde_json::Value) -> Result<MessageId> {
        match &self.mode {
            MessagingMode::Remote { websocket_client, .. } => {
                self.send_cross_app(websocket_client, target_app, topic, content).await
            }
            MessagingMode::Embedded { .. } => {
                Err(LocaiError::Other("Cross-app messaging requires remote mode".to_string()))
            }
        }
    }
    ...
    pub async fn subscribe_cross_app(&self, source_app: &str, topic_pattern: &str) -> Result<MessageStream> {
        match &self.mode {
            MessagingMode::Remote { websocket_client, .. } => {
                self.subscribe_cross_app_remote(websocket_client, source_app, topic_pattern).await
            }
            MessagingMode::Embedded { .. } => {
                Err(LocaiError::Other("Cross-app subscriptions require remote mode".to_string()))
            }
        }
    }
    ...
    pub async fn get_message_history(&self, filter: Option<MessageFilter>, limit: Option<usize>) -> Result<Vec<Message>> {
        match &self.mode {
            MessagingMode::Embedded { memory_manager } => {
                embedded::get_message_history(memory_manager, filter, limit).await
            }
            MessagingMode::Remote { websocket_client, .. } => {
                self.get_message_history_remote(websocket_client, filter, limit).await
            }
        }
    }
    ...
    pub async fn get_process_interactions(&self, process_id: &str) -> Result<Vec<crate::storage::models::Relationship>> {
        match &self.mode {
            MessagingMode::Embedded { memory_manager } => {
                memory_manager.find_related_entities(&format!("process:{}", process_id), None, Some("both".to_string())).await
            ...
            MessagingMode::Remote { .. } => {
                Err(LocaiError::Other("Process interactions query not supported in remote mode".to_string()))
            }
        }
    }
}
...
impl LocaiMessaging {
    async fn send_remote(&self, websocket_client: &Arc<WebSocketClient>, topic: &str, content: serde_json::Value) -> Result<MessageId> {
        remote::send_message(websocket_client, &self.namespace, &self.app_id, topic, content).await
    }
    
    async fn subscribe_remote(&self, websocket_client: &Arc<WebSocketClient>, topic_pattern: &str) -> Result<MessageStream> {
        let filter = MessageFilter {
            topic_patterns: Some(vec![format!("{}.{}", self.namespace, topic_pattern)]),
            ..Default::default()
        };
    ...
    async fn subscribe_filtered_remote(&self, websocket_client: &Arc<WebSocketClient>, filter: MessageFilter) -> Result<MessageStream> {
        remote::subscribe_filtered(websocket_client, filter).await
    }
    
    async fn send_complete_message_remote(&self, websocket_client: &Arc<WebSocketClient>, message: Message) -> Result<MessageId> {
        remote::send_complete_message(websocket_client, message).await
    }
    
    async fn send_cross_app(&self, websocket_client: &Arc<WebSocketClient>, target_app: &str, topic: &str, content: serde_json::Value) -> Result<MessageId> {
        remote::send_cross_app_message(websocket_client, &self.app_id, target_app, topic, content).await
    }
    
    async fn subscribe_cross_app_remote(&self, websocket_client: &Arc<WebSocketClient>, source_app: &str, topic_pattern: &str) -> Result<MessageStream> {
        let filter = MessageFilter {
    ...
    async fn get_message_history_remote(&self, websocket_client: &Arc<WebSocketClient>, filter: Option<MessageFilter>, limit: Option<usize>) -> Result<Vec<Message>> {
