...
impl MemoryAnalyticsEngine {
    ...
    ) -> Result<MemoryEfficiencyMetrics> {
        let memories = self.memory_manager.search_memories("", Some(10000)).await?;
    ...
    fn generate_recommendations(&self, memories: &[Memory]) -> Vec<String> {
        ...
        if tag_utilization < 0.7 {
            recommendations.push(
                "Consider adding more tags to memories to improve discoverability".to_string(),
            );
        }
        ...
        if unique_ratio < 0.8 {
            recommendations
                .push("High content similarity detected - consider deduplication".to_string());
        }

        let type_distribution = self.calculate_type_distribution(memories);
        if type_distribution < 0.3 {
            recommendations
                .push("Consider diversifying memory types for better organization".to_string());
        }
    ...
    async fn detect_anomalies_sync(&self, memories: &[Memory]) -> Result<Vec<MemoryAnomaly>> {
        ...
        let avg_size =
            memories.iter().map(|m| m.content.len()).sum::<usize>() as f32 / memories.len() as f32;
        ...
        for memory in memories {
            if memory.content.len() as f32 > size_threshold {
                anomalies.push(MemoryAnomaly {
                    ...
                    description: format!(
                        "Memory is {:.1} times larger than average",
