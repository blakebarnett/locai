...
impl GraphOperations {
    ...
    pub async fn get_memory_graph(&self, id: &str, depth: u8) -> Result<MemoryGraph> {
        GraphTraversal::get_memory_subgraph(&*self.storage, id, depth).await
    ...
    pub async fn find_paths(&self, from_id: &str, to_id: &str, max_depth: u8) -> Result<Vec<MemoryPath>> {
        GraphTraversal::find_paths(&*self.storage, from_id, to_id, max_depth).await
    ...
    pub async fn find_shortest_path(&self, from_id: &str, to_id: &str, max_depth: u8) -> Result<Option<MemoryPath>> {
        ...
        Ok(paths.into_iter()
            .min_by_key(|path| path.memories.len()))
    }
    ...
    pub async fn find_connected_memories(&self, id: &str, relationship_type: &str, max_depth: u8) -> Result<Vec<Memory>> {
        GraphTraversal::find_connected_memories(&*self.storage, id, relationship_type, max_depth).await
    ...
    pub async fn create_relationship(
        ...
        self.relationship_storage.create_memory_relationship(source_id, target_id, relationship_type).await
    }
    ...
    pub async fn create_bidirectional_relationship(
        ...
        self.relationship_storage.create_bidirectional_memory_relationship(memory_id1, memory_id2, relationship_type).await
    }
    ...
    pub async fn add_related_memory<S: Into<String>>(
        ...
        self.create_relationship(parent_id, &memory_id, relationship_type).await?;
    ...
    pub async fn add_bidirectional_related_memory<S: Into<String>>(
        ...
        self.create_bidirectional_relationship(parent_id, &memory_id, relationship_type).await?;
    ...
    pub async fn get_related_memories(
        ...
        if let Ok(relationships) = self.storage.list_relationships(Some(relationship_filter), Some(100), None).await {
        ...
        if let Ok(entities_in_memory) = self.storage.get_entities_from_memory(memory_id).await {
            for entity in entities_in_memory {
                ...
                if let Ok(related_entities) = self.storage.find_related_entities(
                    &entity.id,
                    ...
                    for related_entity in related_entities {
                        ...
                        if let Ok(related_memory_ids) = self.get_memories_for_entity(&related_entity.id).await {
                            for related_memory_id in related_memory_ids {
                                // Don't include the original memory
                                if related_memory_id != memory_id {
                                    if let Ok(Some(memory)) = self.storage.get_memory(&related_memory_id).await {
        ...
        if relationship_type.is_none() && results.is_empty() {
            ...
            if let Ok(entities_in_memory) = self.storage.get_entities_from_memory(memory_id).await {
                for entity in entities_in_memory.into_iter().take(5) { // Limit to avoid too many results
    ...
    pub async fn get_process_interactions(&self, process_id: &str) -> Result<Vec<Relationship>> {
        ...
        self.storage.find_related_entities(&format!("process:{}", process_id), None, Some("both".to_string())).await
