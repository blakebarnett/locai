...
impl UniversalSearchResult {
    ...
    pub fn summary(&self) -> String {
        match self {
            ...
            Self::Graph { center_id, center_type, graph, .. } => {
                format!("Graph centered on {} {}: {} memories, {} relationships", 
                    center_type, center_id, graph.memories.len(), graph.relationships.len())
            },
        }
    }
}
...
fn reciprocal_rank_fusion(
    ...
    for (rank, (memory, _score)) in text_results.into_iter().enumerate() {
        ...
        scores.entry(memory.id.clone()).and_modify(|s| *s += rrf_score).or_insert(rrf_score);
...
impl SearchExtensions {
    ...
    pub async fn search_with_embedding(
        ...
        match search_mode {
            ...
            SearchMode::Vector => {
                ...
                self.vector_search_with_embedding(query_embedding, limit, filter).await
            }
            SearchMode::Hybrid => {
                // Combine Text and Vector with RRF using query embedding
                self.hybrid_search_with_embedding(query_text, query_embedding, limit, filter).await
            }
        }
    }
    ...
    async fn text_search(
        ...
        let search_results = self.storage.bm25_search_memories(query_text, limit).await
            .map_err(|e| LocaiError::Storage(format!("Failed to perform BM25 search: {}", e)))?;
        
        // Convert to SearchResult format
        Ok(search_results.into_iter().map(|(memory, score, _highlight)| {
            SearchResult {
                memory,
                score: Some(score),
            }
    ...
    async fn vector_search_with_embedding(
        ...
        if let Some(embedding) = query_embedding {
            let search_results = self.storage.vector_search_memories(embedding, limit).await
                .map_err(|e| LocaiError::Storage(format!("Failed to perform vector search with embedding: {}", e)))?;
    ...
    async fn hybrid_search(
        ...
        let text_results = self.storage.bm25_search_memories(query_text, Some(limit * 2)).await
            .map_err(|e| LocaiError::Storage(format!("Failed to perform BM25 search: {}", e)))?;
            
        // Get fuzzy search results for typo tolerance
        let fuzzy_results = self.storage.fuzzy_search_memories(query_text, Some(0.3), Some(limit * 2)).await
            .map_err(|e| LocaiError::Storage(format!("Failed to perform fuzzy search: {}", e)))?;
        
        // Convert to format expected by RRF
        let text_tuples: Vec<(Memory, f32)> = text_results.into_iter()
        ...
        F: FnOnce(MemoryFilter) -> MemoryFilter,
    {
        ...
        self.storage.list_memories(Some(filter), limit, None).await
            .map_err(|e| LocaiError::Storage(format!("Failed to search memories with options: {}", e)))
    }
    ...
    pub async fn universal_search(
        ...
        if options.include_memories {
            let memory_results = self.search_memories_universal(query, Some(limit * 2), &options).await?;
            all_results.extend(memory_results);
        }

        // Search entities if requested
        if options.include_entities {
            let entity_results = self.search_entities_universal(query, Some(limit * 2), &options).await?;
            all_results.extend(entity_results);
        }

        // Search graphs if requested (more expensive)
        if options.include_graphs {
            let graph_results = self.search_graphs_universal(query, Some(limit), &options).await?;
    ...
    async fn search_memories_universal(
        ...
        if query.contains("nonexistent") {
            tracing::debug!("Memory search for '{}' found {} results", query, search_results.len());
            for result in &search_results {
                tracing::debug!("Memory result: {} (score: {:?})", result.memory.id, result.score);
            }
        }
    ...
    async fn search_entities_universal(
        ...
        let entities = self.storage.list_entities(Some(filter), limit, None).await
        ...
        for entity in entities {
            ...
            if query.contains("nonexistent") {
                tracing::debug!("Checking entity {} (name: '{}', type: '{}') against query '{}'", 
                    entity.id, entity_name, entity.entity_type, query);
            }
            ...
            if let Some(description) = entity.properties.get("description")
            ...
            if let Some(props) = entity.properties.as_object() {
                for (key, value) in props {
                    if key != "name" && key != "description" && key != "desc" && key != "text" && key != "value" {
            ...
            if score > 0.0 {
                ...
                if query.contains("nonexistent") {
                    tracing::debug!("FOUND MATCH: entity {} (name: '{}') score: {} reasons: {:?}", 
                        entity.id, entity_name, score, match_reasons);
                }
                ...
                let related_memories = if options.expand_with_relations {
                    self.get_memories_for_entity(&entity.id).await.unwrap_or_default()
                } else {
                    Vec::new()
                };
    ...
    async fn search_graphs_universal(
        ...
        let entity_results = if options.include_entities {
            self.search_entities_universal(query, Some(limit * 2), options).await?
        } else {
            Vec::new()
        };
            ...
            if let UniversalSearchResult::Entity { entity, score, .. } = entity_result {
                ...
                if let Ok(related_memory_ids) = self.get_memories_for_entity(&entity.id).await {
                    if !related_memory_ids.is_empty() {
                        ...
                        if let Ok(relationships) = self.storage.get_entity_relationships(&entity.id).await {
                        ...
                        if graph.memories.len() > 1 || !graph.relationships.is_empty() {
                            universal_results.push(UniversalSearchResult::Graph {
                                ...
                                match_reason: format!("graph centered on entity '{}' matching query", entity.id),
                            });
                        }
                    }
                }
            ...
            if let Ok(graph) = self.get_memory_graph(&memory.id, options.graph_depth).await {
                ...
                if graph.memories.len() > 1 || !graph.relationships.is_empty() {
                    universal_results.push(UniversalSearchResult::Graph {
                        ...
                        match_reason: format!("graph centered on memory matching query"),
                    });
                }
            }
        }
        ...
        universal_results.sort_by(|a, b| b.score().partial_cmp(&a.score()).unwrap_or(std::cmp::Ordering::Equal));
    ...
    async fn get_memories_for_entity(&self, entity_id: &str) -> Result<Vec<String>> {
        ...
        let relationships = self.storage.list_relationships(Some(relationship_filter), Some(100), None).await
            .map_err(|e| LocaiError::Storage(format!("Failed to query relationships: {}", e)))?;
        
        let mut related_memory_ids: Vec<String> = relationships.into_iter()
        ...
        if related_memory_ids.is_empty() {
            ...
            let memories = self.storage.list_memories(Some(memory_filter), Some(100), None).await
    ...
    async fn get_memory_graph(&self, memory_id: &str, depth: u8) -> Result<MemoryGraph> {
        ...
        GraphTraversal::get_memory_subgraph(&*self.storage, memory_id, depth).await
