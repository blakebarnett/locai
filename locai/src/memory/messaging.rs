...
impl MessagingIntegration {
            ...
            match self.storage.setup_live_queries().await {
                Ok(Some(receiver_any)) => {
                    if let Ok(mut receiver) =
                        receiver_any.downcast::<tokio::sync::broadcast::Receiver<DbEvent>>()
                    {
    ...
    pub async fn get_message_history(
        ...
        let memory_filter =
            crate::messaging::filters::convert_message_filter_to_memory_filter(filter)?;
        let memories = self
            .storage
    ...
    fn extract_message_properties(
        ...
        properties.insert(
            "namespace",
            serde_json::Value::String(self.extract_namespace_from_topic(&message.topic)),
        );
...
fn matches_memory_filter_detailed(memory: &Memory, filter: &MemoryFilter) -> bool {
    // Check memory type
