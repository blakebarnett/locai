...
impl MemoryGraphAnalyzer {
    ...
    pub async fn find_shared_memories(
        &self,
        entities: &[String],
    ) -> Result<Vec<Memory>> {
        ...
        let shared: Vec<_> = memories.into_iter()
            .filter(|m| {
                let content_lower = m.content.to_lowercase();
                entities.iter().all(|entity| {
                    content_lower.contains(&entity.to_lowercase())
                })
            })
            .collect();
    ...
    pub async fn detect_memory_communities(
        ...
        let communities = self.cluster_memories(&memories, similarity_threshold).await?;
    ...
    pub async fn traverse_memory_network(
        ...
        let start_memories = self.memory_manager.search_memories(start_memory_id, None).await?;
        ...
        while let Some((current_memory, depth)) = queue.pop_front() {
            ...
            let similar = self.find_similar_memories(&current_memory, similarity_threshold).await?;
    ...
    pub async fn analyze_influence_network(
        ...
        let central_memory = self.memory_manager.search_memories(central_memory_id, None).await?
            .into_iter().next().ok_or_else(|| anyhow::anyhow!("Memory not found"))?;
        ...
        for memory in related_memories {
            if memory.created_at < central_memory.created_at {
                influencing.push(memory.id);
            } else if memory.created_at > central_memory.created_at {
                influenced.push(memory.id);
            }
        }
        
        let network_strength = self.calculate_network_strength(&influencing, &influenced).await?;
    ...
    async fn find_similar_memories(
        ...
        let words: Vec<&str> = target_memory.content.split_whitespace()
            .filter(|w| w.len() > 3)
            .take(5)
            .collect();
        ...
        let candidates = self.memory_manager.search_memories(&search_query, None).await?;
        
        // Filter by similarity threshold
        let similar_memories: Vec<Memory> = candidates.into_iter()
    ...
    async fn find_related_memories(&self, target_memory: &Memory) -> Result<Vec<Memory>> {
        ...
        if !target_memory.tags.is_empty() {
            ...
            let tag_matches = self.memory_manager.search_memories(&tag_query, None).await?;
    ...
    fn select_representative_memories(&self, community: &[Memory]) -> Vec<String> {
        ...
        let mut representatives: Vec<_> = community.iter()
            .map(|memory| {
