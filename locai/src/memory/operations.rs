...
use crate::entity_extraction::{
    EntityExtractor, BasicEntityExtractor, EntityResolver, AutomaticRelationshipCreator,
    ExtractorType,
};
...
impl MemoryOperations {
    ...
    pub fn new(
                ...
                if extractor_config.enabled {
                    match &extractor_config.extractor_type {
                        ExtractorType::Regex => {
                            ...
                            entity_extractors.push(Arc::new(basic_extractor) as Arc<dyn EntityExtractor>);
                            tracing::info!("🔧 Initialized BasicEntityExtractor for structured data");
                        }
                        ExtractorType::Hybrid { .. } => {
                            // Hybrid extractor requires async initialization - defer to new_with_ml
                            tracing::info!("🔧 Hybrid extractor initialization deferred - requires async context");
                        }
                        ExtractorType::Pipeline { .. } => {
                            // Pipeline extractor requires async initialization - defer to new_with_ml
                            tracing::info!("🔧 Pipeline extractor initialization deferred - requires async context");
                        }
                        ExtractorType::Spacy { .. } => {
                            // spaCy extractor requires async initialization - defer to new_with_ml
                            tracing::info!("🔧 spaCy extractor initialization deferred - requires async context");
                        }
                        ExtractorType::HuggingFace { .. } => {
                            // Transformer extractor requires async initialization - defer to new_with_ml
                            tracing::info!("🔧 Transformer extractor initialization deferred - requires async context");
                        }
        ...
        let entity_resolver = if config.entity_extraction.enabled && config.entity_extraction.deduplicate_entities {
            Some(EntityResolver::new(config.entity_extraction.resolution.clone()))
        } else {
            None
        };
        ...
        let relationship_creator = if config.entity_extraction.automatic_relationships.enabled {
            Some(AutomaticRelationshipCreator::new(config.entity_extraction.automatic_relationships.clone()))
        } else {
            None
        };
    ...
    pub async fn store_memory(&self, memory: Memory) -> Result<String> {
        ...
        if let Some(embedding) = &created.embedding {
            ...
            match self.storage.add_vector(vector).await {
                ...
                Err(e) => {
                    tracing::warn!("Failed to create vector record for memory {}: {}", created.id, e);
        ...
        if self.config.entity_extraction.enabled && !self.entity_extractors.is_empty() {
            ...
            for extractor in &self.entity_extractors {
                match extractor.extract_entities(&created.content).await {
                    ...
                    Err(e) => {
                        tracing::warn!("Extractor '{}' failed to extract entities from memory {}: {}", 
                                      extractor.name(), created.id, e);
            ...
            for extracted in all_extracted_entities {
                if extracted.confidence >= self.config.entity_extraction.confidence_threshold {
                    match self.process_extracted_entity_with_resolution(&created.id, &extracted).await {
                        Ok(_) => {
                            tracing::debug!("Successfully processed entity: {}", extracted.format());
                        },
                        Err(e) => {
                    ...
                    tracing::debug!("Skipping entity '{}' due to low confidence: {:.2} < {:.2}", 
                                   extracted.text, extracted.confidence, self.config.entity_extraction.confidence_threshold);
                }
            }
        }
        ...
        if let Some(relationship_creator) = &self.relationship_creator {
            match relationship_creator.create_relationships_for_memory(&created.id, self.storage.as_ref()).await {
                Ok(relationship_ids) => {
                    tracing::debug!("Created {} automatic relationships for memory {}", 
                                   relationship_ids.len(), created.id);
                },
                Err(e) => {
                    tracing::warn!("Failed to create automatic relationships for memory {}: {}", created.id, e);
    ...
    async fn process_extracted_entity_with_resolution(
        ...
        let entity = if let Some(resolver) = &self.entity_resolver {
            ...
            let matches = resolver.find_matches(extracted, self.storage.as_ref()).await?;
            
            if let Some((existing_entity, confidence)) = matches.first() {
                if confidence >= &self.config.entity_extraction.resolution.min_confidence_for_merge {
                    // Merge with existing entity
                    tracing::debug!("Merging entity '{}' with existing entity {} (confidence: {:.2})", 
                                   extracted.text, existing_entity.id, confidence);
                    
                    let merged_entity = resolver.merge_entities(existing_entity.clone(), extracted.clone())?;
                    self.storage.update_entity(merged_entity.clone()).await
                        .map_err(|e| LocaiError::Storage(format!("Failed to update merged entity: {}", e)))?;
    ...
    async fn create_new_entity(&self, extracted: &crate::entity_extraction::ExtractedEntity) -> Result<crate::storage::models::Entity> {
        use crate::storage::models::Entity;
        use std::collections::HashMap;
        
        let mut properties = HashMap::new();
        properties.insert("name".to_string(), serde_json::Value::String(extracted.text.clone()));
        properties.insert("confidence".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(extracted.confidence as f64).unwrap()));
        properties.insert("extractor_source".to_string(), serde_json::Value::String(extracted.extractor_source.clone()));
        properties.insert("start_pos".to_string(), serde_json::Value::Number(serde_json::Number::from(extracted.start_pos)));
        properties.insert("end_pos".to_string(), serde_json::Value::Number(serde_json::Number::from(extracted.end_pos)));
        ...
        let created_entity = self.storage.create_entity(new_entity).await
            .map_err(|e| LocaiError::Storage(format!("Failed to create entity: {}", e)))?;
        
        let entity_name = created_entity.properties
            .get("name")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        tracing::debug!("Created new entity: {} ({})", entity_name, created_entity.id);
    ...
    #[allow(dead_code)]
    async fn process_extracted_entity(&self, memory_id: &str, extracted: &crate::entity_extraction::ExtractedEntity) -> Result<()> {
        // Check if entity already exists or create it
        let existing_entity = self.find_or_create_entity(extracted).await?;
        
        tracing::debug!("Processing entity {} (ID: {}) for memory {}", 
                       extracted.text, existing_entity.id, memory_id);
        
        // Create the "contains" edge: memory -> contains -> entity
        match self.create_contains_edge(memory_id, &existing_entity.id).await {
            Ok(_) => {
                tracing::debug!("Successfully created contains edge: memory {} -> contains -> entity {}", 
                               memory_id, existing_entity.id);
            }
            Err(e) => {
                tracing::warn!("Failed to create 'contains' edge between memory {} and entity {}: {}", 
                              memory_id, existing_entity.id, e);
    ...
    async fn find_or_create_entity(&self, extracted: &crate::entity_extraction::ExtractedEntity) -> Result<crate::storage::models::Entity> {
        ...
        let existing_entities = self.storage.list_entities(Some(filter), Some(1), None).await
        ...
        if let Some(existing) = existing_entities.first() {
            ...
            let entity_name = existing.properties
                .get("name")
                .and_then(|v| v.as_str())
                .unwrap_or("unknown");
    ...
    async fn create_contains_edge(&self, memory_id: &str, entity_id: &str) -> Result<bool> {
        ...
        tracing::debug!("Creating mentions relationship: memory {} -> mentions -> entity {}", 
                       memory_id, entity_id);
        ...
        match self.storage.create_relationship(relationship).await {
            Ok(_) => {
                tracing::debug!("Successfully created contains relationship: memory {} -> entity {}", 
                               memory_id, entity_id);
                Ok(true)
            }
            Err(e) => {
                tracing::warn!("Failed to create contains relationship: memory {} -> entity {}: {}", 
                              memory_id, entity_id, e);
    ...
    pub async fn update_memory(&self, memory: Memory) -> Result<bool> {
        let updated = self.storage.update_memory(memory).await
        ...
        if let Some(embedding) = &updated.embedding {
            ...
            match self.storage.get_vector(&vector_id).await {
                Ok(Some(existing_vector)) => {
                    ...
                    match self.storage.upsert_vector(new_vector).await {
                        ...
                        Err(e) => {
                            tracing::warn!("Failed to update vector record for memory {}: {}", updated.id, e);
                        }
                    }
                },
                Ok(None) => {
                    ...
                    match self.storage.add_vector(vector).await {
                        Ok(_) => {
                            tracing::debug!("Created vector record for updated memory {}", updated.id);
                        },
                        Err(e) => {
                            tracing::warn!("Failed to create vector record for updated memory {}: {}", updated.id, e);
                        }
                    }
                },
                Err(e) => {
                    tracing::warn!("Failed to check for existing vector for memory {}: {}", updated.id, e);
                }
            }
        } else {
            ...
            match self.storage.delete_vector(&vector_id).await {
                Ok(deleted) => {
                    if deleted {
                        tracing::debug!("Deleted vector record for memory {} (no longer has embedding)", updated.id);
                    }
                },
                Err(e) => {
                    tracing::debug!("No vector to delete for memory {} (or deletion failed): {}", updated.id, e);
                }
            }
        }
    ...
    pub async fn count_memories(&self, filter: Option<MemoryFilter>) -> Result<usize> {
        self.storage.count_memories(filter).await
    ...
    pub async fn tag_memory(&self, memory_id: &str, tag: &str) -> Result<bool> {
        ...
        let mut memory = match self.get_memory(memory_id).await? {
            Some(m) => m,
            None => return Err(LocaiError::Memory(format!("Memory with ID {} not found", memory_id))),
        };
