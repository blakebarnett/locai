...
pub fn matches_memory_filter_detailed(
    memory: &Memory,
    filter: &MemoryFilter,
) -> bool {
    // Check memory type
...
pub fn convert_db_event_to_memory(event: &crate::storage::shared_storage::live_query::DbEvent) -> Option<Memory> {
...
pub fn extract_message_properties(message: &crate::messaging::types::Message) -> std::collections::HashMap<String, serde_json::Value> {
    let mut properties = std::collections::HashMap::new();
    properties.insert("message_id".to_string(), serde_json::Value::String(message.id.as_str().to_string()));
    properties.insert("topic".to_string(), serde_json::Value::String(message.topic.clone()));
    properties.insert("namespace".to_string(), serde_json::Value::String(extract_namespace_from_topic(&message.topic)));
    properties.insert("sender".to_string(), serde_json::Value::String(message.sender.clone()));
    properties.insert("recipients".to_string(), serde_json::to_value(&message.recipients).unwrap_or_default());
    properties.insert("timestamp".to_string(), serde_json::to_value(&message.timestamp).unwrap_or_default());
    if let Some(expires_at) = &message.expires_at {
        properties.insert("expires_at".to_string(), serde_json::to_value(expires_at).unwrap_or_default());
    }
    properties.insert("headers".to_string(), serde_json::to_value(&message.headers).unwrap_or_default());
    properties.insert("tags".to_string(), serde_json::to_value(&message.tags).unwrap_or_default());
    properties
} 