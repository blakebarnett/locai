...
pub mod prelude {
    ...
    pub use crate::config::{
        LocaiConfig, ConfigBuilder, StorageConfig, EmbeddingConfig, MLConfig, 
        LogLevel, GraphStorageType, VectorStorageType
    };
    
    // Re-export entity extraction types
    pub use crate::entity_extraction::{
        EntityType, ExtractedEntity, EntityExtractor, BasicEntityExtractor,
        EntityExtractionConfig, ExtractorConfig, ExtractorType,
    };
    
    // Re-export model types
    pub use crate::models::{Memory, MemoryType, MemoryPriority, MemoryBuilder};
    
    // Re-export core types for advanced usage
    pub use crate::core::{MemoryManager, SearchResult, SearchOptions, SearchStrategy, SearchTypeFilter};
    
    // Re-export storage types for advanced usage
    pub use crate::storage::{
        StorageError, models::{MemoryGraph, MemoryPath}
    };
    
    // Re-export essential result type
    pub use crate::{Result, LocaiError};
}
...
pub enum LocaiError {
    ...
    #[error("ML service not configured. To use semantic search, initialize with: Locai::builder().with_defaults().build().await or use ConfigBuilder::new().with_default_ml()")]
    MLNotConfigured,

    /// Storage directory not accessible
    #[error("Storage directory not accessible: {path}. Ensure the directory exists and has write permissions, or try using in-memory storage with Locai::for_testing()")]
    StorageNotAccessible { path: String },

    /// Invalid embedding model
    #[error("Invalid embedding model '{model}'. Try using a supported model like 'BAAI/bge-m3' or initialize with ConfigBuilder::new().with_default_ml()")]
    ...
    /// Empty search query
    #[error("Search query cannot be empty. Provide a meaningful search term like 'what did I learn about science?'")]
    EmptySearchQuery,

    /// No memories found
    #[error("No memories found matching your criteria. Try broadening your search or adding some memories first")]
    NoMemoriesFound,

    /// Feature not enabled
    #[error("Feature '{feature}' is not enabled. Enable it in Cargo.toml with: features = [\"{feature}\"]")]
...
pub async fn init(config: config::LocaiConfig) -> Result<core::MemoryManager> {
    ...
    let storage = storage::create_storage_service(&config).await
