...
async fn test_live_query_event_stream() {
    ...
    manager.simulate_event("memory", "CREATE", test_data.clone()).unwrap();
...
async fn test_live_query_multiple_tables() {
    ...
    manager.simulate_event("memory", "CREATE", memory_data.clone()).unwrap();
    manager.simulate_event("entity", "CREATE", entity_data.clone()).unwrap();
    ...
    let tables: std::collections::HashSet<String> = events.iter().map(|e| e.table.clone()).collect();
