...
mod basic_extraction_tests {
    ...
    async fn test_basic_phone_extraction() {
        ...
        let phone_entity = entities.iter().find(|e| e.entity_type == EntityType::PhoneNumber);
    ...
    async fn test_basic_person_extraction_not_supported() {
        ...
        let person_entity = entities.iter().find(|e| e.entity_type == EntityType::Person);
        assert!(person_entity.is_none(), "BasicEntityExtractor should not extract person entities");
        
        // Verify it only extracts supported types
        for entity in &entities {
            assert!(matches!(entity.entity_type, 
                EntityType::Email | EntityType::Url | EntityType::PhoneNumber | 
    ...
    async fn test_organization_extraction_not_supported() {
        ...
        let org_entity = entities.iter().find(|e| e.entity_type == EntityType::Organization);
        assert!(org_entity.is_none(), "BasicEntityExtractor should not extract organization entities");
        
        // Should only find structured data types (none in this text)
        assert!(entities.is_empty(), "No structured data entities in this text");
    }
    ...
    async fn test_mixed_entity_extraction() {
        ...
        let phone_count = entities.iter().filter(|e| e.entity_type == EntityType::PhoneNumber).count();
        let url_count = entities.iter().filter(|e| e.entity_type == EntityType::Url).count();
        let date_count = entities.iter().filter(|e| e.entity_type == EntityType::Date).count();
        ...
        assert_eq!(person_count, 0, "BasicEntityExtractor should not extract persons");
        assert_eq!(location_count, 0, "BasicEntityExtractor should not extract locations");
        
        // But should extract structured data
        assert!(email_count > 0, "Should find at least one email (found {})", email_count);
        assert!(phone_count > 0, "Should find at least one phone (found {})", phone_count);
        assert!(url_count > 0, "Should find at least one URL (found {})", url_count);
        assert!(date_count > 0, "Should find at least one date (found {})", date_count);
        ...
        for entity in &entities {
            assert!(matches!(entity.entity_type, 
                EntityType::Email | EntityType::Url | EntityType::PhoneNumber | 
                EntityType::Date | EntityType::Time | EntityType::Money
            ), "Found unsupported entity type: {:?}", entity.entity_type);
        }
    }
    ...
    async fn test_confidence_filtering() {
        ...
        for entity in &entities {
            assert!(entity.confidence >= 0.9, "All entities should meet confidence threshold");
        }
    }
    ...
    async fn test_entity_positioning() {
        ...
        let email_entity = entities.iter().find(|e| e.entity_type == EntityType::Email).unwrap();
    ...
    fn test_entity_type_as_str() {
        ...
        assert_eq!(EntityType::Custom("custom_type".to_string()).as_str(), "custom_type");
    }
    ...
    async fn test_basic_entity_extraction_with_types() {
        ...
        let text = "Contact us at support@example.com or visit https://example.com. Call +1-555-123-4567.";
        ...
        if let Some(url) = url_entity {
            ...
            assert!(cleaned_url == "https://example.com" || cleaned_url == "example.com", 
                   "Expected URL to be either full or domain, got: {} (cleaned: {})", url.text, cleaned_url);
        }
...
mod entity_resolution_tests {
    ...
    fn test_merge_strategy_conservative() {
        ...
        existing_properties.insert("name".to_string(), serde_json::Value::String("John".to_string()));
        existing_properties.insert("confidence".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.8).unwrap()));
        existing_properties.insert("email".to_string(), serde_json::Value::String("john@old.com".to_string()));
        ...
        extracted.metadata.insert("phone".to_string(), "555-1234".to_string());
        
        let merged = resolver.merge_entities(existing_entity, extracted).unwrap();
        
        // Should keep existing email (conservative)
        assert_eq!(merged.properties.get("email").unwrap().as_str().unwrap(), "john@old.com");
        // Should add new phone
        assert_eq!(merged.properties.get("phone").unwrap().as_str().unwrap(), "555-1234");
        // Should update confidence - use approximate comparison for floating point
        let confidence = merged.properties.get("confidence").unwrap().as_f64().unwrap();
...
mod relationship_tests {
    ...
    fn test_tag_overlap_calculation() {
        ...
        let tags1 = vec!["rust".to_string(), "programming".to_string(), "web".to_string()];
        let tags2 = vec!["rust".to_string(), "programming".to_string(), "backend".to_string()];
    ...
    fn test_entity_coreference_confidence() {
        ...
        entity_props.insert("name".to_string(), serde_json::Value::String("John".to_string()));
        entity_props.insert("confidence".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.9).unwrap()));
        ...
        let confidence = creator.calculate_entity_coreference_confidence(&memory1, &memory2, &entity);
