...
async fn test_unified_storage_memory_operations() {
    ...
    let memory_id = memory_manager.add_fact("The sky is blue due to Rayleigh scattering")
        .await
    ...
    let conversation_id = memory_manager.add_conversation("User: Hello\nBot: Hi there!")
        .await
    ...
    let retrieved = memory_manager.get_memory(&memory_id).await
    ...
    // Test health check
    let health = memory_manager.storage().health_check().await
    ...
    // Test metadata
    let metadata = memory_manager.storage().get_metadata().await
...
async fn test_unified_storage_with_embedded_database() {
    ...
    let memory_id = memory_manager.add_fact("Testing unified storage with RocksDB")
        .await
...
async fn test_unified_storage_clear_operation() {
    ...
    // Add some memories
    let _id1 = memory_manager.add_fact("First fact").await.expect("Failed to add first fact");
    let _id2 = memory_manager.add_fact("Second fact").await.expect("Failed to add second fact");

    // Clear storage
    memory_manager.clear_storage().await
