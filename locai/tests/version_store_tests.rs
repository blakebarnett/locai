...
use locai::storage::traits::{VersionStore, MemoryStore, EntityStore};
...
async fn create_test_store() -> SharedStorage<surrealdb::engine::local::Db> {
    ...
    let client = surrealdb::Surreal::new::<surrealdb::engine::local::Mem>(()).await.unwrap();
...
async fn test_conversation_version() {
    ...
    assert_eq!(conversation_version.description, "Conversation about AI development");
    assert_eq!(conversation_version.metadata["snapshot_type"], "conversation");
    assert_eq!(conversation_version.metadata["conversation_id"], "conv-123");
    assert_eq!(conversation_version.metadata["context_type"], "ai_assistant");
}
...
async fn test_knowledge_version() {
    ...
    assert_eq!(knowledge_version.description, "Learned about neural networks");
    assert_eq!(knowledge_version.metadata["snapshot_type"], "knowledge");
    assert_eq!(knowledge_version.metadata["topic"], "machine_learning");
    assert_eq!(knowledge_version.metadata["learning_context"], "evolution_tracking");
}
...
async fn test_ai_assistant_conversation_tracking() {
    ...
    let memory2 = create_test_memory("msg2", "AI: Machine learning is a subset of artificial intelligence...");
