...
async fn get_vector_count(locai: &Locai) -> Result<usize> {
    let storage = locai.manager().storage();
    storage.count_vectors(None).await
        .map_err(|e| LocaiError::Storage(format!("Failed to count vectors: {}", e)))
}

#[tokio::test]
async fn test_vector_creation_on_memory_storage() {
    let locai = create_test_locai().await.expect("Failed to create test instance");
    
    // Clear any existing data
    locai.clear_all().await.expect("Failed to clear storage");
    
    // Store a memory that should generate an embedding
    let memory_id = locai.remember_fact("The capital of France is Paris").await
        .expect("Failed to store memory");
    
    // Verify vector was created
    assert!(verify_vector_exists(&locai, &memory_id).await.expect("Failed to check vector"));
    
    // Verify vector count increased
    let vector_count = get_vector_count(&locai).await.expect("Failed to get vector count");
...
async fn test_vector_creation_with_multiple_memories() {
    ...
    for content in memories {
        let id = locai.remember_fact(content).await.expect("Failed to store memory");
        memory_ids.push(id);
    }
    ...
    for memory_id in &memory_ids {
        assert!(verify_vector_exists(&locai, memory_id).await.expect("Failed to check vector"),
                "Vector not found for memory {}", memory_id);
    }
...
async fn test_vector_deletion_on_memory_deletion() {
    ...
    // Store a memory
    let memory_id = locai.remember_fact("This memory will be deleted").await
        .expect("Failed to store memory");
    ...
    let deleted = locai.manager().delete_memory(&memory_id).await
    ...
    let vector_count = get_vector_count(&locai).await.expect("Failed to get vector count");
    assert_eq!(vector_count, 0, "No vectors should remain after memory deletion");
}
...
async fn test_vector_update_on_memory_update() {
    ...
    let memory_id = locai.remember_fact("Initial content").await
        .expect("Failed to store memory");
    ...
    let mut memory = locai.manager().get_memory(&memory_id).await
    ...
    let updated = locai.manager().update_memory(memory).await
    ...
    let vector_count = get_vector_count(&locai).await.expect("Failed to get vector count");
    assert_eq!(vector_count, 1, "Vector count should remain unchanged after update");
}
...
async fn test_semantic_search_with_vector_records() {
    ...
    for content in memory_contents {
        locai.remember_fact(content).await.expect("Failed to store memory");
    }
    ...
    let results = locai.search("email contact information").await
...
async fn test_semantic_search_with_different_queries() {
    ...
    let memories = vec![
        ("Dr. John Smith is a cardiologist at Seattle General Hospital", "medical"),
        ("The annual budget meeting is scheduled for next Tuesday", "business"),
        ("Machine learning algorithms require large datasets for training", "technology"),
        ("The Renaissance period was marked by artistic innovation", "history"),
    ];
    ...
    for (query, _expected_category) in test_cases {
        let results = locai.search(query).await
            .expect("Failed to perform semantic search");
        ...
        assert!(top_result.score > 0.0, 
                "Top result for '{}' should have a positive score", query);
    }
}
...
async fn test_vector_search_performance() {
    ...
    for i in 0..memory_count {
        let content = format!("Memory content number {} with various topics including technology, business, science, and healthcare", i);
        locai.remember_fact(&content).await.expect("Failed to store memory");
    }
    ...
    let results = locai.search("technology business").await
        .expect("Failed to perform semantic search");
    
    let duration = start.elapsed();
    
    // Performance assertions
    assert!(duration.as_millis() < 5000, "Semantic search should complete within 5 seconds");
    assert!(!results.is_empty(), "Should return results");
    assert!(results.len() <= 20, "Should respect default limit parameter of 20");
    
    println!("Semantic search of {} memories took {:?}", memory_count, duration);
}
...
async fn test_vector_storage_error_handling() {
    ...
    // Store a memory
    let memory_id = locai.remember_fact("Test memory for error handling").await
        .expect("Memory storage should succeed even if vector operations fail");
    
    // Verify the memory was stored
    let memory = locai.manager().get_memory(&memory_id).await
        .expect("Failed to get memory")
        .expect("Memory should exist");
...
async fn test_vector_metadata_content() {
    ...
    // Store a memory
    let memory_id = locai.remember_fact("Test memory with metadata").await
        .expect("Failed to store memory");
    
    // Get the vector and check its metadata
    let vector_id = format!("mem_{}", memory_id);
    let storage = locai.manager().storage();
    let vector = storage.get_vector(&vector_id).await
    ...
    assert!(vector.dimension > 0, "Vector should have positive dimensions");
    
    // Check metadata content
    let metadata = vector.metadata;
    assert_eq!(metadata["type"], "memory");
    assert_eq!(metadata["memory_id"], memory_id);
    assert!(metadata["content_preview"].as_str().unwrap().contains("Test memory"));
}
...
async fn test_ai_assistant_conversation_context() {
    ...
    for content in &conversation_memories {
        locai.remember_conversation(*content).await.expect("Failed to store conversation memory");
    }
    
    // AI assistant searches for context when user asks follow-up question
    let context_results = locai.search("machine learning applications resources").await
        .expect("Failed to search for conversation context");
    
    assert!(!context_results.is_empty(), "Should find relevant conversation context");
    
    // Verify we can find multiple relevant memories from the conversation
    let relevant_count = context_results.iter()
        .filter(|r| r.score > 0.1) // Reasonable similarity threshold
        .count();
    
    assert!(relevant_count >= 2, "Should find multiple relevant conversation memories");
}
...
async fn test_knowledge_retrieval_for_ai_responses() {
    ...
    for fact in &knowledge_base {
        locai.remember_fact(*fact).await.expect("Failed to store knowledge");
    }
    
    // AI assistant needs to answer: "What are the main types of machine learning?"
    let results = locai.search("types of machine learning supervised unsupervised reinforcement").await
    ...
    assert!(found_supervised || found_unsupervised || found_reinforcement,
...
async fn test_vector_consistency_across_operations() {
    ...
    let initial_count = get_vector_count(&locai).await.expect("Failed to count vectors");
    assert_eq!(initial_count, 5);
    
    // Delete some memories
    for i in 0..2 {
        locai.manager().delete_memory(&memory_ids[i]).await.expect("Failed to delete memory");
    }
    
    // Verify vector count decreased appropriately
    let after_deletion_count = get_vector_count(&locai).await.expect("Failed to count vectors");
    assert_eq!(after_deletion_count, 3);
    
    // Update remaining memories
    for i in 2..memory_ids.len() {
        let mut memory = locai.manager().get_memory(&memory_ids[i]).await
            .expect("Failed to get memory")
            .expect("Memory should exist");
        
        memory.content = format!("Updated {}", memory.content);
        locai.manager().update_memory(memory).await.expect("Failed to update memory");
    }
    
    // Vector count should remain the same after updates
    let after_update_count = get_vector_count(&locai).await.expect("Failed to count vectors");
    assert_eq!(after_update_count, 3);
    
    // All remaining vectors should still be accessible
    for i in 2..memory_ids.len() {
        assert!(verify_vector_exists(&locai, &memory_ids[i]).await.expect("Failed to check vector"));
    }
} 