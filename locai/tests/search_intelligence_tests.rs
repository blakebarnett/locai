...
async fn create_test_locai() -> Result<(Locai, TempDir)> {
    ...
    let db_path = temp_dir.path().join(format!("test_db_intelligence_{}", test_id));
...
async fn test_search_strategy_selection() {
    ...
    let _memory_ids = setup_intelligence_test_data(&locai).await.unwrap();
     ...
     for strategy in strategies.iter() {
        ...
        let results = locai.search_with_options("neural networks", options).await.unwrap();
...
async fn test_search_with_context() {
    ...
    let results = locai.search_with_options("machine learning algorithms", options).await.unwrap();
    ...
    for result in &results {
        ...
        if has_context {
            println!("Result '{}' has context: entities={}, memories={}, relationships={}", 
                    result.summary(),
...
async fn test_search_type_filtering() {
    ...
    let memory_results = locai.search_with_options("neural networks", memory_options).await.unwrap();
...
async fn test_search_score_thresholds() {
    ...
    let high_threshold_results = locai.search_with_options("machine learning", high_threshold_options).await.unwrap();
    let low_threshold_results = locai.search_with_options("machine learning", low_threshold_options).await.unwrap();
    ...
    for result in &high_threshold_results {
        assert!(result.score >= 0.8, "High threshold result score too low: {}", result.score);
    }
    
    // All low threshold results should meet the threshold  
    for result in &low_threshold_results {
        assert!(result.score >= 0.1, "Low threshold result score too low: {}", result.score);
    }
    
    println!("High threshold (0.8) found {} results", high_threshold_results.len());
    println!("Low threshold (0.1) found {} results", low_threshold_results.len());
}
...
async fn test_search_intent_detection() {
    ...
    for (query, expected_type) in query_types {
        ...
        println!("Query '{}' (expected: {}) found {} results", query, expected_type, results.len());
...
async fn test_conversational_search_context() {
    ...
    assert!(!results1.is_empty(), "Should find results for 'machine learning'");
    println!("First query found {} results", results1.len());
    
    // Follow-up query that might reference previous context
    let results2 = locai.search("how does it work with neural networks").await.unwrap();
    ...
    for (i, results) in [&results1, &results2, &results3].iter().enumerate() {
        for result in *results {
            assert!(!result.id.is_empty(), "Query {} result should have an ID", i + 1);
            assert!(result.score >= 0.0, "Query {} score should be non-negative", i + 1);
        }
    }
}
...
async fn test_search_performance_intelligence() {
    ...
    let results = locai.search("artificial intelligence applications").await.unwrap();
    ...
    assert!(duration.as_millis() < 5000, "Search should complete within reasonable time ({}ms)", duration.as_millis());
    
    println!("Searched {} memories in {}ms, found {} results", memory_ids.len(), duration.as_millis(), results.len());
}
...
async fn test_empty_query_handling() {
    ...
    println!("Whitespace query returned {} results", whitespace_results.len());
}
...
async fn test_concurrent_search_requests() {
    ...
    for handle in handles.into_iter() {
        ...
        println!("Concurrent search {} found {} results", task_id + 1, search_results.len());
