...
async fn test_full_lifecycle_with_hooks() {
    ...
    hook_registry
        .execute_on_accessed(&accessed_memory)
        .await
        .ok();
...
async fn test_custom_relationship_types() {
    ...
    let custom_type = RelationshipTypeDef::new("serves".to_string())
        ...
        .with_custom_metadata("category".to_string(), serde_json::json!("military"));
...
fn test_enhanced_search_scoring() {
    ...
    let frequent_importance_score =
        importance_calc.calculate_final_score(base_score, None, &frequently_accessed);
    let normal_importance_score =
        importance_calc.calculate_final_score(base_score, None, &fresh_memory);
    ...
    let priority_score = importance_calc.calculate_final_score(base_score, None, &high_priority);
