...
async fn test_shared_storage_health_and_metadata() {
    let storage = create_test_storage().await.expect("Failed to create test storage");
    ...
    // Test metadata
    let metadata = storage.get_metadata().await.expect("Failed to get metadata");
...
async fn test_entity_operations() {
    ...
    let created = storage.create_entity(entity.clone()).await.expect("Failed to create entity");
    ...
    let retrieved = storage.get_entity(&created.id).await.expect("Failed to get entity");
    ...
    let updated = storage.update_entity(updated_entity).await.expect("Failed to update entity");
    assert_eq!(updated.properties["value"], 100);

    // Test entity listing
    let entities = storage.list_entities(None, None, None).await.expect("Failed to list entities");
    assert!(!entities.is_empty());
    assert!(entities.iter().any(|e| e.id == created.id));

    // Test entity counting
    let count = storage.count_entities(None).await.expect("Failed to count entities");
    assert!(count > 0);
    ...
    let filtered = storage.list_entities(Some(filter), None, None).await.expect("Failed to filter entities");
    assert!(!filtered.is_empty());
    assert!(filtered.iter().all(|e| e.entity_type == "TestEntity"));

    // Test entity deletion
    let deleted = storage.delete_entity(&created.id).await.expect("Failed to delete entity");
    assert!(deleted);

    // Verify deletion
    let retrieved_after_delete = storage.get_entity(&created.id).await.expect("Failed to get entity after delete");
...
async fn test_relationship_operations() {
    ...
    let created_entity1 = storage.create_entity(entity1).await.expect("Failed to create entity1");
    let created_entity2 = storage.create_entity(entity2).await.expect("Failed to create entity2");
    ...
    let created_rel = storage.create_relationship(relationship).await.expect("Failed to create relationship");
    ...
    let retrieved_rel = storage.get_relationship(&created_rel.id).await.expect("Failed to get relationship");
    ...
    let updated = storage.update_relationship(updated_rel).await.expect("Failed to update relationship");
    assert_eq!(updated.properties["strength"], "very strong");

    // Test relationship listing
    let relationships = storage.list_relationships(None, None, None).await.expect("Failed to list relationships");
    ...
    let filtered = storage.list_relationships(Some(filter), None, None).await.expect("Failed to filter relationships");
    assert!(!filtered.is_empty());
    assert!(filtered.iter().all(|r| r.relationship_type == "knows"));

    // Test finding related entities
    let related = storage.find_related_entities(&created_entity1.id, Some("knows".to_string()), Some("outgoing".to_string())).await.expect("Failed to find related entities");
    assert_eq!(related.len(), 1);
    assert_eq!(related[0].id, created_entity2.id);

    // Test relationship counting
    let count = storage.count_relationships(None).await.expect("Failed to count relationships");
    assert!(count > 0);

    // Test relationship deletion
    let deleted = storage.delete_relationship(&created_rel.id).await.expect("Failed to delete relationship");
    assert!(deleted);

    // Verify deletion
    let retrieved_after_delete = storage.get_relationship(&created_rel.id).await.expect("Failed to get relationship after delete");
    assert!(retrieved_after_delete.is_none());
}
...
async fn test_vector_operations() {
    ...
    let created = storage.add_vector(vector).await.expect("Failed to add vector");
    assert_eq!(created.dimension, 1024);
    assert_eq!(created.metadata["title"], "Test Document");

    // Test vector retrieval
    let retrieved = storage.get_vector(&created.id).await.expect("Failed to get vector");
    assert!(retrieved.is_some());
    let retrieved = retrieved.unwrap();
    ...
    let updated = storage.update_vector_metadata(&created.id, new_metadata).await.expect("Failed to update vector metadata");
    ...
    storage.add_vector(vector2).await.expect("Failed to add vector2");
    storage.add_vector(vector3).await.expect("Failed to add vector3");
    ...
    let results = storage.search_vectors(&query_vector, search_params).await.expect("Failed to search vectors");
    assert!(!results.is_empty());
    assert!(results.len() <= 3);

    // Verify results are sorted by distance (best matches first)
    for i in 1..results.len() {
        assert!(results[i-1].1 <= results[i].1, "Results should be sorted by distance (ascending)");
    }

    // Test vector listing
    let vectors = storage.list_vectors(None, None, None).await.expect("Failed to list vectors");
    ...
    let filtered = storage.list_vectors(Some(filter), None, None).await.expect("Failed to filter vectors");
    assert_eq!(filtered.len(), 1); // Should find only vec_003, since vec_001 was updated to category "updated_test"

    // Test vector counting
    let count = storage.count_vectors(None).await.expect("Failed to count vectors");
    assert_eq!(count, 3);
    ...
    let batch_results = storage.batch_add_vectors(batch_vectors).await.expect("Failed to batch add vectors");
    ...
    storage.upsert_vector(upsert_vector.clone()).await.expect("Failed to upsert vector");
    
    // Verify upsert created the vector
    let upserted = storage.get_vector("upsert_test").await.expect("Failed to get upserted vector");
    assert!(upserted.is_some());

    // Test vector deletion
    let deleted = storage.delete_vector(&created.id).await.expect("Failed to delete vector");
    assert!(deleted);

    // Verify deletion
    let retrieved_after_delete = storage.get_vector(&created.id).await.expect("Failed to get vector after delete");
    assert!(retrieved_after_delete.is_none());
}
...
async fn test_clear_storage() {
    ...
    storage.create_entity(entity).await.expect("Failed to create test entity");
    storage.add_vector(vector).await.expect("Failed to add test vector");

    // Verify data exists
    let entity_count_before = storage.count_entities(None).await.expect("Failed to count entities");
    let vector_count_before = storage.count_vectors(None).await.expect("Failed to count vectors");
    ...
    let entity_count_after = storage.count_entities(None).await.expect("Failed to count entities after clear");
    let vector_count_after = storage.count_vectors(None).await.expect("Failed to count vectors after clear");
    assert_eq!(entity_count_after, 0);
    assert_eq!(vector_count_after, 0);

    // Verify storage is still healthy
    let health = storage.health_check().await.expect("Failed to check health after clear");
    assert!(health);
}
...
async fn test_complex_relationship_queries() {
    ...
    let author_entity = storage.create_entity(author).await.expect("Failed to create author");
    let paper_entity = storage.create_entity(paper).await.expect("Failed to create paper");
    let topic_entity = storage.create_entity(topic).await.expect("Failed to create topic");
    ...
    storage.create_relationship(authorship).await.expect("Failed to create authorship");
    storage.create_relationship(topic_relation).await.expect("Failed to create topic relation");

    // Test finding relationships by entities
    let author_relationships = storage.find_relationships(&author_entity.id, &paper_entity.id, Some("authored".to_string())).await.expect("Failed to find author relationships");
    ...
    let rel_properties = storage.get_relationship_properties(&author_relationships[0].id).await.expect("Failed to get relationship properties");
    assert_eq!(rel_properties["year"], 2024);

    // Test bidirectional relationship finding
    let related_papers = storage.find_related_entities(&author_entity.id, Some("authored".to_string()), Some("outgoing".to_string())).await.expect("Failed to find related papers");
    assert_eq!(related_papers.len(), 1);
    assert_eq!(related_papers[0].id, paper_entity.id);

    let related_topics = storage.find_related_entities(&paper_entity.id, Some("covers".to_string()), Some("outgoing".to_string())).await.expect("Failed to find related topics");
