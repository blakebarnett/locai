...
async fn main() -> Result<()> {
    ...
    let game_master =
        LocaiMessaging::embedded(shared_memory.clone(), "game_master".to_string()).await?;

    let player_elara =
        LocaiMessaging::embedded(shared_memory.clone(), "player_elara".to_string()).await?;

    let player_thorne =
        LocaiMessaging::embedded(shared_memory.clone(), "player_thorne".to_string()).await?;
    ...
    let gm_filter = MessageFilter::new().senders(vec!["game_master".to_string()]);

    let gm_messages = game_master
        .get_message_history(Some(gm_filter), Some(10))
        .await?;
    for (i, msg) in gm_messages.iter().enumerate() {
        println!(
            ...
            msg.content
                .get("text")
                .or(msg.content.get("description"))
    ...
    let action_filter = MessageFilter::new().topic_patterns(vec!["*.character.action".to_string()]);

    let action_messages = shared_memory
        .get_message_history(&action_filter, Some(10))
        .await?;
    ...
    if let Some(memory_manager) = game_master.memory_manager() {
        let interactions = memory_manager
            .get_process_interactions("game_master")
            .await?;
        println!(
            "🔗 Game Master agent interactions: {} relationships",
            interactions.len()
        );

        let elara_interactions = memory_manager
            .get_process_interactions("player_elara")
            .await?;
        println!(
            "🔗 Player Elara agent interactions: {} relationships",
            elara_interactions.len()
        );
    }
    ...
    let important_messages = game_master
        .get_message_history(Some(important_filter), Some(5))
        .await?;
    ...
    let dialogue_filter =
        MessageFilter::new().topic_patterns(vec!["*.character.dialogue.*".to_string()]);

    let dialogue_messages = game_master
        .get_message_history(Some(dialogue_filter), Some(10))
        .await?;
