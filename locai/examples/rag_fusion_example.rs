...
use locai::prelude::*;
use locai::models::{MemoryType, Memory};
...
pub trait LlmInterface: Send + Sync {
    async fn generate_response(&self, prompt: &str) -> std::result::Result<String, Box<dyn std::error::Error + Send + Sync>>;
    async fn generate_queries(&self, original_query: &str, num_variants: usize) -> std::result::Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>>;
    async fn verify_factuality(&self, response: &str, context: &[String]) -> std::result::Result<f32, Box<dyn std::error::Error + Send + Sync>>;
    async fn verify_instruction_following(&self, response: &str, instruction: &str) -> std::result::Result<f32, Box<dyn std::error::Error + Send + Sync>>;
    async fn refine_response(&self, response: &str, issues: &[String], context: &[String]) -> std::result::Result<String, Box<dyn std::error::Error + Send + Sync>>;
}
...
impl LlmInterface for MockLlm {
    async fn generate_response(&self, prompt: &str) -> std::result::Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if prompt.contains("quantum") {
            ...
            Ok(format!("This is a mock response to the query: {}", prompt.chars().take(50).collect::<String>()))
        }
    }
    ...
    async fn verify_factuality(&self, response: &str, _context: &[String]) -> std::result::Result<f32, Box<dyn std::error::Error + Send + Sync>> {
        // Mock factuality scoring based on response characteristics
        let score = if response.contains("scientific evidence") || response.contains("research shows") {
            0.9
    ...
    async fn refine_response(&self, response: &str, issues: &[String], _context: &[String]) -> std::result::Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let mut refined = response.to_string();
        
        if issues.iter().any(|issue| issue.contains("factuality")) {
            refined.push_str(" [Note: This information has been verified against reliable sources.]");
        }
        
        if issues.iter().any(|issue| issue.contains("instruction")) {
            refined.push_str(" [Note: Response has been adjusted to better follow the given instructions.]");
        }
...
impl RagFusionSystem {
    pub fn new(memory_manager: Arc<MemoryManager>, llm: Arc<dyn LlmInterface>, config: RagFusionConfig) -> Self {
        Self {
            memory_manager,
            llm,
            config,
    ...
    pub async fn generate_query_variants(&self, original_query: &str) -> Result<Vec<String>> {
        println!("🔄 Generating {} query variants...", self.config.num_query_variants);
        
        let variants = self.llm.generate_queries(original_query, self.config.num_query_variants)
            .await
    ...
    pub async fn multi_query_retrieval(&self, queries: &[String]) -> Result<Vec<RetrievedMemory>> {
        ...
        for (_query_idx, query) in queries.iter().enumerate() {
            ...
            let search_results = self.memory_manager
                .search(query, Some(self.config.retrieval_limit_per_query), None, SearchMode::Text)
                .await?;
    ...
    pub fn apply_reciprocal_rank_fusion(&self, results: Vec<RetrievedMemory>) -> Vec<RetrievedMemory> {
        ...
        for (i, result) in final_results.iter().enumerate() {
            println!("    {}. [Score: {:.3}] {}", i + 1, result.relevance_score, 
    ...
    pub async fn generate_response(&self, query: &str, context: &[RetrievedMemory]) -> Result<String> {
        ...
        println!("  Generated response: {}", response.chars().take(100).collect::<String>());
        Ok(response)
    }

    /// Verify response using agentic verification system
    pub async fn verify_response(&self, response: &str, instruction: &str, context: &[String]) -> Result<VerificationResult> {
        ...
        let factuality_score = self.llm.verify_factuality(response, context)
            .await
            .map_err(|e| LocaiError::Memory(format!("Factuality verification failed: {}", e)))?;
        
        // Instruction following verification
        let instruction_following_score = self.llm.verify_instruction_following(response, instruction)
            .await
            .map_err(|e| LocaiError::Memory(format!("Instruction following verification failed: {}", e)))?;
        ...
        println!("  Factuality: {:.2}, Instruction Following: {:.2}, Overall: {:.2}", 
    ...
    pub async fn refine_response(&self, response: &str, verification: &VerificationResult, context: &[String]) -> Result<String> {
        ...
        let refined = self.llm.refine_response(response, &verification.issues, context)
            .await
            .map_err(|e| LocaiError::Memory(format!("Response refinement failed: {}", e)))?;
        
        println!("  Refined response: {}", refined.chars().take(100).collect::<String>());
        Ok(refined)
    }
    ...
    pub async fn process_query(&self, query: &str) -> Result<String> {
        ...
        let context_strings: Vec<String> = fused_results.iter()
        ...
        for iteration in 0..self.config.max_refinement_iterations {
            ...
            let verification = self.verify_response(&response, query, &context_strings).await?;
            
            if verification.overall_score >= 0.8 {
                println!("✅ Response quality acceptable (score: {:.2})", verification.overall_score);
                break;
            }
            
            if iteration < self.config.max_refinement_iterations - 1 {
                response = self.refine_response(&response, &verification, &context_strings).await?;
            }
        }
    ...
    pub async fn store_memory(&self, content: &str, memory_type: MemoryType, tags: Vec<String>) -> Result<String> {
...
async fn seed_knowledge_base(memory_manager: &MemoryManager) -> Result<()> {
    ...
    for (content, memory_type, tags) in knowledge_items {
        let id = memory_manager.add_memory_with_options(
            content.to_string(),
            |builder| {
                let mut b = builder.memory_type(memory_type);
                for tag in tags {
        ...
        println!("  Added: {} (ID: {})", content.chars().take(50).collect::<String>(), id);
    }
