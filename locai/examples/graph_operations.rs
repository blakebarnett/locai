...
async fn main() -> Result<()> {
    ...
    let sky_id = memory_manager.add_fact(
    ...
    let scattering_id = memory_manager.add_related_memory(
        &sky_id,
    ...
    info!("Added related fact about Rayleigh scattering, ID: {}", scattering_id);
    
    let sun_id = memory_manager.add_related_memory(
        &sky_id,
    ...
    memory_manager.create_bidirectional_relationship(
        &scattering_id,
        &sun_id,
        "connected_to"
    ).await?;
    ...
    info!("Memory graph contains {} memories and {} relationships",
        graph.memories.len(),
    ...
    let explained_by = memory_manager.find_connected_memories(
        &sky_id,
        "explains",
        2
    ).await?;
    
    info!("Found {} memories that explain the sky memory", explained_by.len());
    ...
    let weather_id = memory_manager.add_memory_with_options(
    ...
    memory_manager.create_relationship(&sky_id, &weather_id, "related_to").await?;
    ...
    let high_priority = memory_manager.filter_memories(
        filter,
        Some("created_at"),
