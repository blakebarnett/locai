...
impl MockFastembedModel {
    ...
    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        println!("🧠 [Mock] Generating {} local embeddings with {}", 
                 texts.len(), self.model_name);
        
        let embeddings: Vec<Vec<f32>> = texts.iter().map(|text| {
            // Mock embedding generation
            (0..self.dimensions)
                .map(|i| {
                    let base = (i as f32 * 0.003 + text.len() as f32 * 0.02).sin();
                    // Add some text-specific variation
                    base + (text.chars().nth(i % text.len()).unwrap_or('a') as u8 as f32 * 0.001)
                })
                .collect()
        }).collect();
...
async fn main() -> Result<()> {
    ...
    let embedding_manager = EmbeddingManager::with_expected_dimensions(embedding_model.dimensions());
    ...
    println!("\n📚 Batch Processing {} Documents with Local Embeddings", documents.len());
    println!("=======================================================");
    ...
    println!("✅ Generated {} embeddings in {:?}", embeddings.len(), batch_duration);
    println!("   Average time per embedding: {:?}", batch_duration / documents.len() as u32);
    ...
    for (i, (doc, embedding)) in documents.iter().zip(embeddings.iter()).enumerate() {
        ...
        if i < 3 {  // Show first few for brevity
            println!("   ✅ Memory {}: {}", i + 1, doc.chars().take(50).collect::<String>());
        }
    }
    ...
    for query in test_queries {
        ...
        println!("      ✅ Generated local query embedding ({} dims)", query_embedding.len());
        println!("      💡 Ready for semantic similarity search");
        println!("      📊 Would compare against {} stored embeddings", memory_ids.len());
    }
    ...
    println!("Batch embedding generation (3 texts): {:?}", batch_test_duration);
    println!("Batch efficiency: {:.1}x faster per text", 
    ...
    let memories_with_embeddings = locai.recent_memories(Some(1000)).await?
        .iter()
