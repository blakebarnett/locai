...
async fn test_filter_by_importance(messaging: &EmbeddedMessaging) -> Result<()> {
    ...
    let filter = MessageFilter::new().importance_range(0.8, 1.0); // High importance only

    let messages = messaging
        .get_message_history(Some(filter), Some(10))
        .await?;
...
async fn test_filter_by_tags(messaging: &EmbeddedMessaging) -> Result<()> {
    ...
    let filter = MessageFilter::new().tags(vec!["urgent".to_string()]);
...
async fn demo_message_history(messaging: &EmbeddedMessaging) -> Result<()> {
    ...
    let player_filter = MessageFilter::new().senders(vec!["player1".to_string()]);

    let player_messages = messaging
        .get_message_history(Some(player_filter), Some(10))
        .await?;
