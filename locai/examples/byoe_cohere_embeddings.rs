...
impl MockCohereClient {
    ...
    async fn embed_text(&self, text: &str, input_type: &str) -> Result<Vec<f32>> {
        ...
        println!(
            "🧠 [Mock] Generating Cohere embedding ({}): '{}'",
            input_type,
...
async fn main() -> Result<()> {
    ...
    for (i, doc) in knowledge_docs.iter().enumerate() {
        ...
        println!(
            "   ✅ Document {}: {}",
            i + 1,
            doc.chars().take(60).collect::<String>()
        );
    }
    ...
    for query in queries {
        ...
        println!(
            "      ✅ Generated Cohere query embedding ({} dims)",
            query_embedding.len()
        );
        ...
        println!(
            "      📊 Would compare with {} stored document embeddings",
            memory_ids.len()
        );
    }
    ...
    // 8. Memory statistics
    println!("\n📊 Final Statistics:");
    let all_memories = locai.recent_memories(Some(1000)).await?;
    let cohere_memories = all_memories
        .iter()
