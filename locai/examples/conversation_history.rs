...
use locai::prelude::*;
use locai::models::{MemoryType, MemoryPriority, Memory};
...
impl ConversationManager {
    ...
    pub async fn create_conversation(&mut self, title: &str, participants: Vec<String>) -> Result<String> {
        ...
        self.active_conversations.insert(conversation_id.clone(), thread);
        
        println!("📝 Created conversation '{}' with ID: {}", title, conversation_id);
        Ok(conversation_id)
    }
    ...
    pub async fn add_message(
        ...
        println!("💬 Added message from {} to conversation {}", participant_name, conversation_id);
        Ok(memory_id)
    }

    /// Retrieve conversation context for generating responses
    pub async fn get_conversation_context(&self, conversation_id: &str, limit: Option<usize>) -> Result<Vec<Memory>> {
    ...
    pub async fn get_relevant_memories(&self, conversation_id: &str, query: &str) -> Result<Vec<Memory>> {
        println!("🔍 Searching for relevant memories for query: {}", query);
        
        // First, get memories from this conversation
        let conversation_memories = self.get_conversation_context(conversation_id, Some(5)).await?;
        
        // Then, search for semantically similar memories across all conversations
        let relevant_memories = self.memory_manager
            .search(query, Some(self.config.context_retrieval_limit), None, SearchMode::Text)
            .await?;
    ...
    pub async fn summarize_conversation(&mut self, conversation_id: &str) -> Result<String> {
        ...
        let participants: std::collections::HashSet<String> = context.iter()
            .filter_map(|m| {
        ...
        let summary_id = self.memory_manager.add_memory_with_options(
            summary.clone(),
            |builder| {
                builder
                    .memory_type(MemoryType::Conversation)
    ...
    pub async fn get_conversation_stats(&self, conversation_id: &str) -> Result<ConversationStats> {
        ...
        Ok(ConversationStats {
            ...
            total_words,
            participant_counts,
            duration_hours: thread.map(|t| {
                (t.last_activity - t.created_at).num_hours() as f64
            }).unwrap_or(0.0),
...
async fn demonstrate_conversation_management() -> Result<()> {
    ...
    let conversation_id = conv_manager.create_conversation(
        "AI Discussion",
    ...
    let conversation_turns = vec![
        ("user_1", "Alice", "Hello! I'm interested in learning about artificial intelligence.", MessageType::User),
        ("assistant", "AI Assistant", "Hello Alice! I'd be happy to help you learn about AI. What specific aspects interest you most?", MessageType::Assistant),
        ("user_1", "Alice", "I'm curious about machine learning and how it differs from traditional programming.", MessageType::User),
        ("assistant", "AI Assistant", "Great question! Traditional programming involves writing explicit instructions, while machine learning allows computers to learn patterns from data.", MessageType::Assistant),
        ("user_1", "Alice", "That's fascinating! Can you give me an example of machine learning in everyday life?", MessageType::User),
        ("assistant", "AI Assistant", "Certainly! Email spam filters are a common example. They learn to identify spam by analyzing patterns in millions of emails.", MessageType::Assistant),
        ("user_1", "Alice", "I see! What about deep learning? How does that fit into machine learning?", MessageType::User),
    ];
    ...
    let context = conv_manager.get_conversation_context(&conversation_id, Some(5)).await?;
    ...
    let relevant = conv_manager.get_relevant_memories(&conversation_id, "deep learning").await?;
    ...
    let stats = conv_manager.get_conversation_stats(&conversation_id).await?;
    ...
    for conv in conversations {
        println!("  - {} (ID: {}, {} messages)", conv.title, conv.id, conv.message_count);
    }
    
    // Create another conversation to demonstrate multi-conversation management
    println!("\n🆕 Creating second conversation...");
    let conv2_id = conv_manager.create_conversation(
        "Technical Support",
