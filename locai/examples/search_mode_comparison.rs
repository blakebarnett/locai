    ...
    async fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let embedding: Vec<f32> = (0..self.dimensions)
            .map(|i| {
                ...
                let char_influence =
                    text.chars().nth(i % text.len()).unwrap_or('a') as u8 as f32 * 0.001;
                base + char_influence
            })
            .collect();
...
async fn main() -> Result<()> {
    ...
    let documents = [
        ...
        (
            "science",
            "Quantum mechanics describes the fundamental behavior of matter and energy at atomic scales",
        ),
        (
            "business",
            "Market analysis reveals consumer preferences and drives strategic decision making",
        ),
        (
            "health",
            "Regular exercise and balanced nutrition are essential for maintaining optimal health",
        ),
        (
            "education",
            "Effective teaching methods adapt to different learning styles and student needs",
        ),
        (
            "environment",
            "Climate change mitigation requires global cooperation and sustainable practices",
        ),
        (
            "art",
            "Renaissance paintings demonstrate masterful use of perspective and light techniques",
        ),
        (
            "technology",
        ...
        (
            "psychology",
            "Cognitive behavioral therapy helps individuals modify negative thought patterns",
        ),
        (
            "history",
        ...
        (
            "economics",
            "Supply and demand dynamics determine pricing in competitive markets",
        ),
        (
            "medicine",
            "Precision medicine uses genetic information to customize treatment approaches",
        ),
        (
            "philosophy",
            "Existentialism explores the meaning of individual existence and personal freedom",
        ),
        (
            "biology",
    ...
    println!(
        "\n📚 Creating Knowledge Base with {} Documents",
        documents.len()
    );
    println!("==============================================");
    ...
    for service in [&openai_service, &cohere_service, &local_service] {
        ...
        println!(
            "   {} ({} dims): {:?}",
            service.name, service.dimensions, duration
        );
    }
    ...
    for (mode, benchmarks) in mode_stats {
        let avg_duration: f64 = benchmarks
            .iter()
            .map(|b| b.duration.as_millis() as f64)
            .sum::<f64>()
            / benchmarks.len() as f64;

        let avg_results: f64 = benchmarks
            .iter()
    ...
    let daily_embedding_cost =
        (queries_per_day as f64 / 1000.0) * avg_tokens_per_query as f64 * openai_cost_per_1k_tokens;

    println!("Example cost analysis for {} queries/day:", queries_per_day);
    println!("   Text Search: $0.00 (no embedding costs)");
    println!(
        "   Vector Search: ${:.4}/day (OpenAI embeddings)",
        daily_embedding_cost
    );
    println!(
        "   Hybrid Search: ${:.4}/day (embedding cost same as vector)",
        daily_embedding_cost
    );
