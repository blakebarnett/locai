...
pub async fn handle_quickstart_command(
    ...
    if args.step.is_none() {
        println!("{}", "━━━ Locai Quick Start ━━━".color(CliColors::accent()).bold());
        println!();
    ...
    let sample_memories = vec![
        ("The protagonist is a skilled warrior named John", locai::models::MemoryType::Fact, locai::models::MemoryPriority::High),
        ("John met Alice in the tavern last week", locai::models::MemoryType::Episodic, locai::models::MemoryPriority::Normal),
        ("The kingdom has been at war for three years", locai::models::MemoryType::World, locai::models::MemoryPriority::Normal),
        ("Alice is a skilled mage who studies ancient magic", locai::models::MemoryType::Identity, locai::models::MemoryPriority::Normal),
        ("The tavern is located in the capital city", locai::models::MemoryType::World, locai::models::MemoryPriority::Low),
    ];
        ...
        let id = if let Some(existing_memory) = existing.first() {
            ...
            ctx
                ...
                .add_memory_with_options(
                    content.to_string(),
                    |builder| builder.memory_type(mem_type).priority(priority),
                )
                .await?
        };
        ...
        if memory_index < 3
            ...
            let embedding = if let Some(emb) = embeddings.get(content) {
                ...
                if emb.len() == 1024 {
                    ...
                    tracing::debug!("Pre-generated embedding has {} dimensions, need 1024. Using mock embedding.", emb.len());
    ...
    if args.step.is_none() {
        ...
        let embeddings = load_quickstart_embeddings();
        let has_real_embeddings = !embeddings.is_empty() && embeddings.values().next().map(|v| v.len() == 1024).unwrap_or(false);
        ...
        if embedding_count > 0 {
            println!("{}", format_success(&format!("✓ Created/verified {} sample memories ({} with {} for semantic search demo)", created_ids.len(), embedding_count, embedding_source)));
        } else {
            println!("{}", format_success(&format!("✓ Created/verified {} sample memories", created_ids.len())));
        }
    ...
    if ctx.memory_manager.get_entity("entity:quickstart:john").await?.is_none() {
    ...
    if ctx.memory_manager.get_entity("entity:quickstart:alice").await?.is_none() {
    ...
    if args.step.is_none() {
        println!("{}", format_success(&format!("✓ Created/verified {} sample entities", entity_count)));
    ...
    if created_ids.len() >= 2 {
        ...
        if memory1.is_some() && memory2.is_some() {
            ...
            let existing = ctx.memory_manager.list_relationships(Some(filter), Some(1), None).await?;
            if existing.is_empty() {
                match ctx.memory_manager
        ...
        let john_entity = ctx.memory_manager.get_entity("entity:quickstart:john").await?;
        ...
        let alice_entity = ctx.memory_manager.get_entity("entity:quickstart:alice").await?;
        if alice_entity.is_some() && memory2.is_some() {
            ...
            if existing.is_empty() {
                match ctx.memory_manager
                    .create_relationship(&created_ids[1], "entity:quickstart:alice", "has_character")
                    .await {
                    Ok(_) => relationship_count += 1,
                    Err(e) => {
                        if !e.to_string().contains("not found") {
    ...
    if args.step.is_none() {
        if relationship_count > 0 {
            let msg = if skipped_count > 0 {
                format!("✓ Created/verified {} sample relationships ({} skipped)", relationship_count, skipped_count)
            } else {
                format!("✓ Created/verified {} sample relationships", relationship_count)
            };
            println!("{}", format_success(&msg));
        } else {
            println!("{}", format_warning("⚠ No relationships created (entities may not be ready yet)"));
        }
    }
    ...
    match args.step {
        Some(1) => {
            println!();
            println!("{}", "━━━ Step 1: Search ━━━".color(CliColors::accent()).bold());
            println!();
            ...
            println!("  • First 3 memories have {} embeddings for demo", "mock".color(CliColors::accent()));
            println!("  • Try semantic search: {}", "locai-cli memory search \"character\" --mode semantic".color(CliColors::accent()));
            println!();
            println!("Next: {}", "locai-cli quickstart --step 2".color(CliColors::accent()));
        }
        Some(2) => {
            println!();
            println!("{}", "━━━ Step 2: Explore ━━━".color(CliColors::accent()).bold());
            println!();
            ...
            println!("Next: {}", "locai-cli quickstart --step 3".color(CliColors::accent()));
        }
        Some(3) => {
            println!();
            println!("{}", "━━━ Step 3: Learn More ━━━".color(CliColors::accent()).bold());
            println!();
        ...
        Some(n) if n > 3 => {
            println!();
            println!("{}", format_error("Invalid step number. Use --step 1, 2, or 3."));
        }
        _ => {
            ...
            println!("  • Run interactive tutorial: {}", "locai-cli tutorial".color(CliColors::accent()));
            println!("  • Learn concepts: {}", "locai-cli --explain memory".color(CliColors::accent()));
            println!("  • Step-by-step guide: {}", "locai-cli quickstart --step 1".color(CliColors::accent()));
            println!("  • Remove sample data: {}", "locai-cli quickstart --cleanup".color(CliColors::accent()));
            println!();
            println!("{}", "💡 About Semantic Search:".bold());
            let embeddings = load_quickstart_embeddings();
            let has_real_embeddings = !embeddings.is_empty() && embeddings.values().next().map(|v| v.len() == 1024).unwrap_or(false);
            if has_real_embeddings {
                println!("  • First 3 memories have {} embeddings for demonstration", "pre-generated".color(CliColors::accent()));
            } else {
                println!("  • First 3 memories have {} embeddings for demonstration", "mock".color(CliColors::accent()));
                println!("  • To use real embeddings, run: {}", "./scripts/generate_quickstart_embeddings.sh".color(CliColors::accent()));
            }
            println!("  • Try: {}", "locai-cli memory search \"character\" --mode semantic".color(CliColors::accent()));
            ...
            println!("  • See: {}", "locai-cli --explain search".color(CliColors::accent()));
        }
    }
...
async fn cleanup_quickstart_data(ctx: &LocaiCliContext) -> locai::Result<()> {
    println!("{}", "━━━ Cleaning Up Quickstart Data ━━━".color(CliColors::accent()).bold());
    println!();
    println!("{}", format_info("Removing sample memories and entities..."));
    ...
    println!();
    println!("{}", format_success(&format!("✓ Cleaned up {} memories and quickstart entities", deleted_count)));
    println!();
    println!("{}", format_info("Sample data removed. Run 'locai-cli quickstart' again to recreate it."));
