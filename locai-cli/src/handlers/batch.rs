...
pub async fn handle_batch_command(
    ...
    match cmd {
        BatchCommands::Execute(args) => {
            ...
            let (operations, file_transaction): (Vec<BatchOperation>, Option<bool>) = if let Ok(ops_array) = serde_json::from_str::<Vec<serde_json::Value>>(&file_contents) {
                (ops_array
                    .into_iter()
                    .filter_map(|v| serde_json::from_value::<BatchOperation>(v).ok())
                    .collect(), None)
            } else if let Ok(batch_obj) = serde_json::from_str::<serde_json::Value>(&file_contents) {
                if let Some(ops_array) = batch_obj.get("operations").and_then(|v| v.as_array()) {
                    let file_transaction = batch_obj.get("transaction").and_then(|v| v.as_bool());
                    (ops_array
                        .iter()
                        .filter_map(|v| serde_json::from_value::<BatchOperation>(v.clone()).ok())
                        .collect::<Vec<_>>(), file_transaction)
                } else {
                    return Err(LocaiError::Other(
                        "Batch file must contain 'operations' array or be an array of operations".to_string(),
                    ));
                }
            } else {
                return Err(LocaiError::Other("Invalid JSON format in batch file".to_string()));
            };

            if operations.is_empty() {
                return Err(LocaiError::Other("No operations found in batch file".to_string()));
            }
            ...
            let pb = if atty::is(atty::Stream::Stdout) && output_format != "json" && operations.len() > 5 {
            ...
            let response = if let Some(ref progress_bar) = pb {
                ...
                progress_bar.finish_with_message(format!("Completed {} operations", response.completed));
                response
            } else {
                executor
                    .execute(operations.clone(), transaction)
