...
use locai_server::websocket::{WebSocketMessage, MemoryFilter};
...
async fn test_websocket_subscription() {
    ...
    ws_sender.send(Message::Text(subscription_json.into())).await.unwrap();
    ...
    if let Message::Text(text) = msg {
        let ws_msg: WebSocketMessage = serde_json::from_str(&text).expect("Invalid JSON");
        match ws_msg {
            WebSocketMessage::SubscriptionAck { filters_applied, message } => {
...
async fn test_websocket_ping_pong() {
    ...
    ws_sender.send(Message::Text(ping_json.into())).await.unwrap();
