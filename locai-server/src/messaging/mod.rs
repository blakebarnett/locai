...
impl MessagingStorage {
    ...
    pub async fn store_message(&self, message: &Message) -> Result<()> {
        ...
        memory.set_property("recipients", serde_json::to_value(&message.recipients).unwrap_or_default());
        memory.set_property("headers", serde_json::to_value(&message.headers).unwrap_or_default());
        memory.set_property("tags", serde_json::to_value(&message.tags).unwrap_or_default());
        memory.set_property("message_type", serde_json::Value::String("messaging".to_string()));
        if let Some(expires_at) = message.expires_at {
            memory.set_property("expires_at", serde_json::to_value(expires_at).unwrap_or_default());
        }
        ...
        self.shared_storage.create_memory(memory).await
    ...
    pub async fn get_message_history(
        ...
        let memories = self.shared_storage.list_memories(Some(memory_filter), limit, None).await
...
fn convert_message_filter_to_memory_filter(
    filter: Option<MessageFilter>,
) -> Result<MemoryFilter> {
    ...
    if let Some(f) = filter {
        ...
        properties.insert("message_type".to_string(), serde_json::Value::String("messaging".to_string()));
...
fn convert_memory_to_message(memory: Memory) -> Result<Message> {
    ...
    let recipients = properties.get("recipients")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
        .unwrap_or_default();
    
    let headers = properties.get("headers")
    ...
    let expires_at = properties.get("expires_at")
        .and_then(|v| serde_json::from_value(v.clone()).ok());
