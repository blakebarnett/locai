...
impl MessagingServer {
    ...
    pub fn new_with_shared_storage(config: MessagingConfig, shared_storage: &Arc<dyn locai::storage::traits::GraphStore>) -> Self {
    ...
    pub async fn authenticate_connection(&self, connection_id: &str, app_id: &str) -> Result<bool> {
        ...
        if let Some(app_info) = connections.get_mut(connection_id) {
            if app_info.app_id == app_id {
                ...
                info!("Authenticated connection {} for app {}", connection_id, app_id);
                return Ok(true);
            }
        }
        
        warn!("Failed to authenticate connection {} for app {}", connection_id, app_id);
        Ok(false)
    }
    ...
    pub async fn send_message(
        ...
        debug!("Sent message {} from app {} to topic {}", message_id.as_str(), sender_app, topic);
        Ok(message_id)
    }
    ...
    pub async fn subscribe(
        ...
        info!("Created subscription {} for app {} with filter: {:?}", subscription_id, app_id, filter);
    ...
    fn message_matches_filter(message: &Message, filter: &MessageFilter) -> bool {
        ...
        if let Some(patterns) = &filter.topic_patterns {
            if !patterns.iter().any(|pattern| Self::topic_matches_pattern(&message.topic, pattern)) {
                return false;
            }
        }
    ...
    pub async fn get_stats(&self) -> HashMap<String, serde_json::Value> {
        ...
        stats.insert("active_connections".to_string(), serde_json::Value::Number(connections.len().into()));
        stats.insert("active_subscriptions".to_string(), serde_json::Value::Number(subscriptions.len().into()));
        stats.insert("messaging_enabled".to_string(), serde_json::Value::Bool(self.config.enabled));
        stats.insert("cross_app_enabled".to_string(), serde_json::Value::Bool(self.config.enable_cross_app));
