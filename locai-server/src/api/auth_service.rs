...
use locai::{
    core::MemoryManager,
    storage::{filters::EntityFilter, models::Entity},
};

use crate::{
    api::auth::{generate_jwt_token, hash_password, verify_password},
...
impl AuthService {
    ...
    pub async fn initialize(
        ...
        let _user = self
            .create_user(memory_manager, "root", &password, "root", None)
            .await?;
        ...
        if !was_provided {
            tracing::warn!(
                "IMPORTANT: Save the root password above! Set LOCAI_ROOT_PASSWORD environment variable to customize it."
            );
        }
    ...
    pub async fn authenticate(
        ...
        // Get user by username
        let user = self
            .get_user_by_username(memory_manager, username)
            .await?
    ...
    pub async fn delete_user(
        ...
        storage
            .delete_entity(&user_id.to_string())
            .await
