...
use crate::{
    api::dto::{CreateRelationshipRequest, RelationshipDto, EntityDto},
...
pub async fn create_relationship(
    ...
    if !source_exists {
        return Err(ServerError::BadRequest(format!("Source entity '{}' not found", request.source_id)));
    }
    ...
    if !target_exists {
        return Err(ServerError::BadRequest(format!("Target entity '{}' not found", request.target_id)));
    }
