...
use locai::{Locai, MemoryBuilder, SearchMode};
...
async fn main() -> Result<()> {
    ...
    for (query_type, query) in test_queries {
        ...
        if let Some(first) = text_results.first() {
            println!("      Top result: [Score: {:.3}] {}", 
                     ...
                     first.memory.content.chars().take(50).collect::<String>());
        }
    ...
    let mut mode_stats: HashMap<SearchMode, Vec<&SearchBenchmark>> = HashMap::new();
    for benchmark in &all_benchmarks {
        mode_stats.entry(benchmark.mode).or_default().push(benchmark);
    }
